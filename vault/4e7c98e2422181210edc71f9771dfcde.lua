-- French Translation
-- Script ID: 4e7c98e2422181210edc71f9771dfcde
-- Updated: 2026-09-14T17:51:35.559Z
...
local Tab = Window:AddTab("Voler", "shopping-bag")
local Box = Tab:AddLeftGroupbox("Voler des œufs")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Zones" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Raretés" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mutations" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Priorité de cible" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Méthode de vol", Tooltip = "Après le coup du poulet : Instant TP se colle à l'œuf (le plus rapide) ; Second Method y court par la voie comme l'ancien auto-vol (sans téléport)" })
Box:AddToggle("se_selected",   { Text = "Vol automatique", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Étapes auto (choisissez ce que le farm fait)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "Ne pas vendre (raretés)" })
Box:AddToggle("se_return",     { Text = "Retour auto à la base", Default = true })
Box:AddButton({ Text = "Quitter le tapis", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Infos")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Vol : Actif")
InfoBox:AddButton({ Text = "Rescanner les zones", Func = function()
    local areas = {}
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Buggé") .. " -> réinitialisation respawn")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop : API bloquée par l'exécuteur (0 serveur)" or "Hop : aucun serveur rejoignable, réessayer")
        hopping = false
        return
    end
    ...
    notify(("Hop : %d serveurs, le plus vide %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop : tout a échoué, réessayer"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("Aucune batte à équiper") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Batte équipée via l'emplacement hotbar " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("Les touches hotbar n'ont pas équipé - gardez la fenêtre Roblox active et réessayez") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Mouvement")
MoveBox:AddToggle("se_tpwalk", { Text = "Vitesse", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Vitesse de marche", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Vol", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Vitesse de vol", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Anti-recul", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Anti-coup (reprise si lâché)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "ESP d'œufs (tous les œufs)", Default = false })
MoveBox:AddButton({ Text = "suicide", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Réinitialisation (respawn) en cas d'échec", Default = true, Tooltip = "Quand un cycle de vol bug et te bloque sur le terrain, réapparais à la base (via suicide) au lieu de revenir lentement -- redémarrage bien plus rapide" })

local ServerBox = Tab:AddRightGroupbox("Serveur")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Préférence de saut" })
ServerBox:AddButton({ Text = "Changer de serveur", Func = function()
    local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
    task.spawn(function() serverHop(pref) end)
end })
ServerBox:AddButton({ Text = "Rejoindre", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "Auto-Hop quand plein", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Hop à joueurs >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Quand l'Auto-Hop est activé et que le serveur atteint ce nombre de joueurs, saute vers un nouveau serveur (utilise la préférence de saut)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Serveur plein (" .. #Players:GetPlayers() .. ") -> saut")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "EN PAUSE - FPS bas (" .. fps .. ")"
            elseif not stealAllowed() then disp = "En pause (réinit. des œufs)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "Actif"
            else disp = "Inactif" end
            stateLabel:SetText("Statut : " .. tostring(disp))
        end)
        task.wait(0.5)
    end
end)
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("Tapis introuvable"); return end
    setStatus("En route vers le tapis")
    ...
    onTread = true
    setStatus("Sur le tapis (aucun œuf)")
end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("En pause (réinit. des œufs)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("En attente d'œufs") end
                return
            end
            ...
            if not fr then setStatus("Aucun œuf de forêt à amorcer"); unlockMove(); return end
            setStatus("Amorçage : œuf de forêt")
            ...
            setStatus("En attente du coup du poulet...")
            ...
            if not hit then setStatus("Aucun coup détecté en 25s"); resetToBase("pas de coup de poulet"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("Échec de la prise de la cible"); resetToBase("échec de la prise"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("Œuf lâché (anti-coup désactivé)"); break end
                        setStatus("Œuf lâché -> j'y retourne")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("échec du retour") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("Récupération...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("Événements", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("Grande Floraison")
BloomBox:AddToggle("se_bloom", { Text = "Auto-farm Floraison", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("Machine de fusion")
FuseBox:AddToggle("se_autofuse", { Text = "Auto-fusion des familiers", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Incubateur Sakura")
local crysLabel = IncBox:AddLabel("Cristaux : 0")
IncBox:AddToggle("se_incubate", { Text = "Auto-incubation", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Auto-dépôt de cristaux", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Auto-mutation quand plein", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Événement Boss")
BossBox:AddToggle("se_boss", { Text = "Auto-Boss (entrer + frapper les cristaux)", Default = false })
local bossLabel = BossBox:AddLabel("Boss : inactif")
local bossCountLabel = BossBox:AddLabel("Prochain boss : --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Boss : entrée") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Boss : cristaux (" .. #crystals .. " restants)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Boss : attaque des bras (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Boss : attaque du boss (aucun bras)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Boss : attente (changement de phase)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("Cristaux : " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Boss : ACTIF (dans l'arène)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Prochain boss dans ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Lumière vs Ténèbres")
    LvdBox:AddToggle("se_lvd", { Text = "Auto Lumière vs Ténèbres", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Équipe" })
    local lvdLabel = LvdBox:AddLabel("LvD : inactif")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Lumière vs Ténèbres COMMENCÉ -- " .. fmtT(dur) .. " restant", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD : rejoint l'équipe") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " restant") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " anneaux" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD : en attente (déclenché par le staff, sans minuteur)" or "LvD : inactif") end)
                task.wait(1)
            end
        end
    end)
    ...
    while RUNNING do
        local ok = pcall(function()
            ...
            for _, tree in ipairs(CollectionService:GetTagged("SakuraBloomTree")) do
                ...
                if okp and pos then
                    lockMove()
                    setStatus("Floraison : frappe l'arbre")
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Floraison : collecte du cristal")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("Aucun trio à fusionner"); return end
                setStatus("Fusion des familiers")
...
ProtBox:AddToggle("se_antilag", { Text = "Réduire le lag", Default = false, Tooltip = "Supprime particules/ombres/effets, basse qualité, et MASQUE les familiers des autres joueurs (côté client) pour stopper le pic de lag au téléport", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Zones")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Noclip des gardes", Default = true, Tooltip = "Rend les animaux gardiens traversables (côté client) pour qu'ils ne te bloquent ni te poussent pendant le vol", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "Verrouiller l'heure" })
...
local FB_ERRMAP = {
    ...
    rate_limited = "Vous avez atteint la limite de 3 messages. Réessayez plus tard.",
    busy = "Le serveur est occupé. Réessayez.", banned = "Vous n'êtes pas autorisé à envoyer des retours.",
    discord_error = "L'envoi a échoué. Réessayez.", network = "Erreur réseau.",
    no_request = "Votre exécuteur ne peut pas envoyer de requêtes.", bad_response = "Le serveur a renvoyé une réponse inattendue.",
    unreachable = "Votre exécuteur ne peut pas joindre le serveur (erreur TLS). Essayez un autre exécuteur.",
}
...
local FG = SuggestTab:AddLeftGroupbox("Suggestions / Aide", "message-square")
FG:AddDropdown("FeedbackType", { Text = "Type", Values = { "Suggestion", "Bug", "Help" }, Default = "Suggestion", Callback = function() end })
FG:AddInput("FeedbackMsg", { Default = "", Numeric = false, Finished = false, ClearTextOnFocus = false, Text = "Message", Placeholder = "Tapez votre message (500 caractères maximum, sans liens)" })
...
FG:AddButton({
    ...
    Func = function()
        ...
        if #msg > 500 then pcall(function() fbStatus:SetText("Le message est trop long (500 caractères maximum).") end); return end
        local low = msg:lower()
        ...
        task.spawn(function()
            ...
            if sok then
                ...
                pcall(function() fbStatus:SetText(FB_ERRMAP[err] or ("Échec de l'envoi : " .. tostring(err))) end)
            end
        end)
    end,
})
FG:AddLabel("Maximum 3 messages par heure. Aucun lien ni mention.", true)

local HelpGroup = SuggestTab:AddRightGroupbox("Aide / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Rejoindre le Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Lien Discord copié. Collez-le dans votre navigateur.")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("Une suggestion ou un bug trouvé ?\nChoisissez un type, tapez votre message et appuyez sur Envoyer.", true)

notify("COMBO_WICK chargé")
