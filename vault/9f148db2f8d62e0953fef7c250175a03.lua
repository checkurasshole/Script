...
-- Script ID: 9f148db2f8d62e0953fef7c250175a03
-- Updated: 2026-09-14T17:51:34.405Z
...
local Tab = Window:AddTab("Nakawan", "shopping-bag")
local Box = Tab:AddLeftGroupbox("Nakawin ang mga Itlog")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Mga Lugar" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mga Rarity" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mga Mutasyon" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Priyoridad ng Target" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Paraan ng Pagnanakaw", Tooltip = "Pagkatapos ng hit ng manok: Instant TP ay dumidikit agad sa itlog (pinakamabilis); Second Method ay tumatakbo dito sa lane gaya ng lumang auto-steal (walang teleport)" })
Box:AddToggle("se_selected",   { Text = "Awtomatikong Pagnanakaw", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Auto Steps (piliin ang gagawin ng farm)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "Huwag Ibenta (rarity)" })
Box:AddToggle("se_return",     { Text = "Awtomatikong Balik sa Base", Default = true })
Box:AddButton({ Text = "Umalis sa Treadmill", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Impormasyon")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Nakaw: Aktibo")
InfoBox:AddButton({ Text = "I-rescan ang mga Lugar", Func = function()
    local areas = {}
...
-- Respawn-reset: a clean SUICIDE respawns us at base when a cycle strands us in the field -- far faster than
-- tweening back from a stuck/airborne spot. Guarded so we never suicide off a valuable carry.
local function resetToBase(reason)
    if not on("se_suicidereset") then return end
    setStatus((reason or "Na-bug") .. " -> reset via respawn")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    hopping = true
    local lowest = pref ~= "Highest Players"
    ...
    for _ = 1, 50 do
        ...
        if lowest and #list >= 40 and list[1].playing <= 2 then break end
        if data.nextPageCursor and data.nextPageCursor ~= "" then cursor = data.nextPageCursor else break end
        task.wait(0.12)
    end
    if #list == 0 then
        notify(scanned == 0 and "Hop: hinarang ng executor ang API (0 server)" or "Hop: walang server na mapapasukan, subukan ulit")
        hopping = false
        return
    end
    if not lowest then table.sort(list, function(a, b) return a.playing > b.playing end) end
    local order = {}
    if lowest then
    ...
    notify(("Hop: %d server, pinaka-walang tao %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: lahat nabigo, subukan ulit"); return end
    ...
    tryNext()
    task.delay(30, function() if conn then conn:Disconnect() end; hopping = false end)
end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("Walang bat na maisusuot") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Naisuot ang bat via hotbar slot " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("Hindi naisuot ng hotbar keys - panatilihing naka-focus ang Roblox window at subukan ulit") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Paggalaw")
MoveBox:AddToggle("se_tpwalk", { Text = "Walkspeed", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Bilis ng Paglakad", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Paglipad", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Bilis ng Paglipad", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Anti Knockback", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Anti Hit (kunin ulit kapag nahulog)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "Egg ESP (lahat ng itlog)", Default = false })
MoveBox:AddButton({ Text = "pagpapakamatay", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Respawn Reset kapag Nabigo", Default = true, Tooltip = "Kapag nag-bug ang steal cycle at naiwan ka sa field, mag-respawn sa base (via suicide) sa halip na dahan-dahang bumalik -- mas mabilis mag-restart" })

local ServerBox = Tab:AddRightGroupbox("Server")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Kagustuhan sa Hop" })
...
ServerBox:AddButton({ Text = "Sumali Muli", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "Auto Hop kapag puno", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Mag-hop sa players >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Kapag naka-on ang Auto Hop at umabot ang server sa dami ng player na ito, lilipat sa bagong server (gumagamit ng Hop Preference)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Puno ang server (" .. #Players:GetPlayers() .. ") -> nagho-hop")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "NAKA-PAUSE - mababang FPS (" .. fps .. ")"
            elseif not stealAllowed() then disp = "Naka-pause (egg reset)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "Aktibo"
            else disp = "Idle" end
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("Walang nahanap na Treadmill"); return end
    setStatus("Papunta sa treadmill")
    ...
    onTread = true
    setStatus("Nasa treadmill (walang itlog)")
end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("Naka-pause (egg reset)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("Naghihintay ng itlog") end
                return
            end
            ...
            if not fr then setStatus("Walang forest egg na maihahanda"); unlockMove(); return end
            setStatus("Ihanda: forest egg")
            ...
            setStatus("Naghihintay ng hit ng manok...")
            ...
            if not hit then setStatus("Walang hit na na-detect sa 25s"); resetToBase("walang tama ng manok"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("Nabigong makuha ang target"); resetToBase("nabigong makuha"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("Nahulog ang itlog (anti-hit off)"); break end
                        setStatus("Nahulog ang itlog -> babalikan ito")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("nabigong bumalik") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("Nagre-recover...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("Mga Event", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("Great Bloom")
BloomBox:AddToggle("se_bloom", { Text = "Auto Farm Bloom", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("Fuse Machine")
FuseBox:AddToggle("se_autofuse", { Text = "Auto Fuse ng mga Alaga", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Sakura Incubator")
local crysLabel = IncBox:AddLabel("Kristal: 0")
IncBox:AddToggle("se_incubate", { Text = "Auto Incubate", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Auto Deposit ng Kristal", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Auto Mutate Kapag Puno", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Boss Event")
BossBox:AddToggle("se_boss", { Text = "Auto Boss (pasok + hampasin ang kristal)", Default = false })
local bossLabel = BossBox:AddLabel("Boss: idle")
local bossCountLabel = BossBox:AddLabel("Susunod na boss: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Boss: pumapasok") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Boss: kristal (" .. #crystals .. " natitira)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Boss: inaatake ang mga braso (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Boss: inaatake ang boss (walang braso)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Boss: naghihintay (phase change)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("Kristal: " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Boss: AKTIBO (nasa arena)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Susunod na boss sa ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
-- LIGHT vs DARKNESS event auto-farm. The game auto-collects rings/power-ups when our character is near them,
-- so we join a team, sweep the field (power-ups first), and claim milestones (100/800/1500). Staff-triggered
-- event -- no fixed timer; we only show time REMAINING once it starts. Do-block keeps helpers off the register limit.
do
    local LvdBox = EventsTab:AddLeftGroupbox("Liwanag vs Dilim")
    LvdBox:AddToggle("se_lvd", { Text = "Auto Liwanag vs Dilim", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Koponan" })
    local lvdLabel = LvdBox:AddLabel("LvD: idle")
    local LVD = Remotes and Remotes.LightVsDarkness
    local LE = Remotes and Remotes.LiveEvents
    local lvdEndAt = 0
    local function fmtT(s) s = math.floor(math.max(s, 0)); return ("%d:%02d"):format(math.floor(s / 60), s % 60) end
    local function lvdRemain() return lvdEndAt - Workspace:GetServerTimeNow() end
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                lvdEndAt = Workspace:GetServerTimeNow() + dur
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Nagsimula ang Liwanag vs Dilim -- " .. fmtT(dur) .. " natitira", Time = 6 }) end)
            end
        end)) end)
        pcall(function() track(LE.Ended.OnClientEvent:Connect(function(id) if id == "LightVsDarkness" then lvdEndAt = 0 end end)) end)
        task.spawn(function() local ok, run = pcall(function() return LE.FetchRunning:InvokeServer() end); if ok and type(run) == "table" and type(run.LightVsDarkness) == "number" then lvdEndAt = run.LightVsDarkness end end)
    end
    local function lvdActive() return Workspace:FindFirstChild("LightVsDarknessRings") ~= nil or type(lp:GetAttribute("LvdTeam")) == "string" end
    local function lvdTeam() local t = lp:GetAttribute("LvdTeam"); return type(t) == "string" and t or nil end
    local function pivotOf(o)
        if o:IsA("BasePart") then return o.Position end
        local ok, cf = pcall(function() return o:GetPivot() end); if ok and cf then return cf.Position end
        local bp = o:FindFirstChildWhichIsA("BasePart", true); return bp and bp.Position or nil
    end
    local function nearestIn(folder, me)
        if not folder then return nil, math.huge end
        local best, bd = nil, math.huge
        for _, o in ipairs(folder:GetChildren()) do local p = pivotOf(o); if p then local d = (p - me).Magnitude; if d < bd then best, bd = p, d end end end
        return best, bd
    end
    local function lvdTarget()
        local h = hrp(); if not h then return nil end
        local me = h.Position
        local pw = nearestIn(Workspace:FindFirstChild("LightVsDarknessPowerUps"), me)
        if pw then return pw end
        return (nearestIn(Workspace:FindFirstChild("LightVsDarknessRings"), me))
    end
    task.spawn(function()
        local claimed = {}
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    if LVD and LVD.AskSelectTeam then pcall(function() LVD.AskSelectTeam:FireServer((Opt.se_lvdteam and Opt.se_lvdteam.Value) or "Random") end) end
                    pcall(function() lvdLabel:SetText("LvD: sumasali sa koponan") end); task.wait(0.5)
                else
                    local rc = lp:GetAttribute("LvdRingsCollected") or 0
                    for i, thr in ipairs({ 100, 800, 1500 }) do
                        if not claimed[i] and rc >= thr then
                            if LVD and LVD.AskClaimMilestone then pcall(function() LVD.AskClaimMilestone:FireServer(i) end) end
                            claimed[i] = true
                        end
                    end
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " natitira") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " singsing" .. suffix) end)
                    local tgt = lvdTarget()
                    if tgt then Nav.tweenTo3D(tgt + Vector3.new(0, 3, 0), function() return on("se_lvd") and lvdActive() end, (Opt.se_walk and Opt.se_walk.Value) or 974)
                    else task.wait(0.4) end
                end
            else
                if next(claimed) then claimed = {} end
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: naghihintay (staff ang nagsimula, walang timer)" or "LvD: idle") end)
                task.wait(1)
            end
        end
    end)
    task.spawn(function()
        local pausedByLvd = false
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if Toggles.se_selected and Toggles.se_selected.Value and not pausedByLvd then
                    pausedByLvd = true; pcall(function() Toggles.se_selected:SetValue(false) end)
                end
            elseif pausedByLvd then
                pausedByLvd = false; pcall(function() if Toggles.se_selected then Toggles.se_selected:SetValue(true) end end)
            end
            task.wait(0.5)
        end
    end)
    ...
    while RUNNING do
        local ok = pcall(function()
            ...
            for _, tree in ipairs(CollectionService:GetTagged("SakuraBloomTree")) do
                ...
                if okp and pos then
                    lockMove()
                    setStatus("Bloom: hinahampas ang puno")
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Bloom: kinukuha ang kristal")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("Walang tatlong magkakapareho na i-fuse"); return end
                setStatus("Fina-fuse ang mga alaga")
...
local ProtBox = SettingsTab:AddLeftGroupbox("Proteksyon")
ProtBox:AddToggle("se_antiafk", { Text = "Anti AFK", Default = true })
ProtBox:AddToggle("se_antilag", { Text = "Bawasan ang Lag", Default = false, Tooltip = "Pinapatay ang particles/shadows/effects, mababang quality, at ITINATAGO ang mga alaga ng ibang players (client-side) para pigilan ang teleport lag spike", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Mga Lugar")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Guard Noclip", Default = true, Tooltip = "Ginagawang natatagusan ang guard animals (client-side) para hindi ka nila maharang o matulak habang nagnanakaw", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "I-lock ang Oras" })
...
MenuBox:AddButton({ Text = "I-unload", Func = function()
...
MenuBox:AddLabel("I-toggle ang Menu"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "I-toggle ang Menu" })
...
local FG = SuggestTab:AddLeftGroupbox("Suhestiyon / Tulong", "message-square")
    ...
    Func = function()
        ...
        task.spawn(function()
            ...
            if sok then
                ...
                notify("Naipadala ang feedback mo. Salamat!")
            else
...
local HelpGroup = SuggestTab:AddRightGroupbox("Tulong / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Sumali sa Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Nakopya ang Discord link. I-paste sa iyong browser.")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("May suhestiyon o nakakita ng bug?\nPumili ng uri, i-type ang mensahe, at pindutin ang Ipadala.", true)

notify("Na-load ang COMBO_WICK")
