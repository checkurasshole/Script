-- Russian Translation
-- Script ID: 8eb832ee71baeed32a3083b83e3b4c60
-- Updated: 2025-12-18T07:42:24.460Z
...
local Tabs = {
    Respawn = Window:AddTab("Авто Возрождение", "refresh-cw"),
    AimAssist = Window:AddTab("Помощник Прицеливания", "star"),
    Visuals = Window:AddTab("Визуалы", "eye"),
    ["UI Settings"] = Window:AddTab("Настройки UI", "settings"),
}

local AutoPlayBox = Tabs.Respawn:AddLeftGroupbox("Цикл Авто Игры")
local AimbotV1Box = Tabs.AimAssist:AddLeftGroupbox("Версия 1 - Ручной ПК")
local AimbotV2Box = Tabs.AimAssist:AddRightGroupbox("Версия 2 - Автоматический")
local TriggerBox = Tabs.AimAssist:AddLeftGroupbox("Авто Выстрел")
local ESPBox = Tabs.Visuals:AddLeftGroupbox("ESP Игрока")
local TracerBox = Tabs.Visuals:AddRightGroupbox("Трассеры")
...
AutoPlayBox:AddToggle("AutoPlay", {
    Text = "Цикл Авто Игры",
    Default = false,
    Tooltip = "Авто возрождение при смерти",
})
...
AimbotV1Box:AddToggle("AimbotV1", {
    Text = "Включить",
    Default = false,
    Tooltip = "Удерживайте правую кнопку для блокировки",
})
...
Options.AimbotV1Part:OnChanged(function()
    local mapping = {["Голова"] = "Head", ["Торс"] = "Torso", ["Центр"] = "HumanoidRootPart"}
...
AimbotV2Box:AddToggle("AimbotV2", {
    Text = "Включить",
    Default = false,
    Tooltip = "Авто блокировка без кнопки",
})
...
TriggerBox:AddToggle("TriggerBot", {
    Text = "Включить",
    Default = false,
    Tooltip = "Авто выстрел при блокировке",
})
                ...
                if player ~= LocalPlayer and Module.isAlive(player) then
                    ...
                    if hrp then
                        ...
                        if onScreen then
                            ...
                            local fromPos
                            if Options.TracerPosition.Value == "Верх" then
                                fromPos = Vector2.new(Camera.ViewportSize.X / 2, 0)
                            elseif Options.TracerPosition.Value == "Середина" then
