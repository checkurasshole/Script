...
-- Script ID: 79b975caca615f76380ea8aa97570f66
-- Updated: 2026-09-14T17:51:33.331Z
...
local Tab = Window:AddTab("偷取", "shopping-bag")
local Box = Tab:AddLeftGroupbox("偷取蛋")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "区域" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "稀有度" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "变异" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "目标优先级" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "偷取方式", Tooltip = "被鸡击中后：Instant TP 瞬间贴到蛋上（最快）；Second Method 像旧版自动偷取那样沿通道跑过去（无传送）" })
Box:AddToggle("se_selected",   { Text = "自动偷取", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "自动步骤（选择farm执行的操作）" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "不出售（稀有度）" })
Box:AddToggle("se_return",     { Text = "自动返回基地", Default = true })
Box:AddButton({ Text = "离开跑步机", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("信息")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("偷取: 进行中")
InfoBox:AddButton({ Text = "重新扫描区域", Func = function()
    local areas = {}
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: 执行器屏蔽了 API（0 个服务器）" or "Hop: 没有可加入的服务器，重试")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d 个服务器，最空 %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: 全部失败，重试"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("没有可装备的球棒") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("通过快捷栏槽装备球棒 " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("快捷栏按键未能装备 - 保持 Roblox 窗口聚焦并重试") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("移动")
MoveBox:AddToggle("se_tpwalk", { Text = "移动速度", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "行走速度", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "飞行", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "飞行速度", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "防击退", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "防击落（掉落则重新抓取）", Default = true })
MoveBox:AddToggle("se_esp", { Text = "蛋透视（所有蛋）", Default = false })
MoveBox:AddButton({ Text = "自杀", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "失败时重生重置", Default = true, Tooltip = "当偷取循环出错把你困在场地时，直接在基地重生（通过自杀）而不是缓慢移动回去 -- 重启快得多" })

local ServerBox = Tab:AddRightGroupbox("服务器")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "跳服偏好" })
ServerBox:AddButton({ Text = "跳转服务器", Func = function()
    local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
    task.spawn(function() serverHop(pref) end)
end })
ServerBox:AddButton({ Text = "重新加入", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "满员时自动跳服", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "玩家数 >= 时跳服", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "当 Auto Hop 开启且服务器达到该玩家数时，跳到新服务器（使用跳服偏好）" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("服务器已满 (" .. #Players:GetPlayers() .. ") -> 跳服中")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "已暂停 - FPS 过低 (" .. fps .. ")"
            elseif not stealAllowed() then disp = "已暂停（蛋重置）"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "进行中"
            else disp = "空闲" end
            stateLabel:SetText("状态: " .. tostring(disp))
        end)
        task.wait(0.5)
    end
end)
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("未找到跑步机"); return end
    setStatus("前往跑步机")
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("已暂停（蛋重置）"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("等待蛋") end
                return
            end
            ...
            if not fr then setStatus("没有可预备的森林蛋"); unlockMove(); return end
            setStatus("预备: 森林蛋")
            ...
            if not hit then setStatus("25秒内未检测到攻击"); resetToBase("未被鸡击中"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("抓取目标失败"); resetToBase("抓取失败"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("蛋掉落（防击落关）"); break end
                        setStatus("蛋掉落 -> 返回去捡")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("返回失败") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("恢复中...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("活动", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("盛大绽放")
BloomBox:AddToggle("se_bloom", { Text = "自动farm绽放", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("融合机")
FuseBox:AddToggle("se_autofuse", { Text = "自动融合宠物", Default = false })
local IncBox = EventsTab:AddRightGroupbox("樱花孵化器")
local crysLabel = IncBox:AddLabel("水晶: 0")
IncBox:AddToggle("se_incubate", { Text = "自动孵化", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "自动存入水晶", Default = true })
IncBox:AddToggle("se_automutate", { Text = "满时自动变异", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Boss 活动")
BossBox:AddToggle("se_boss", { Text = "自动 Boss（进入 + 击打水晶）", Default = false })
local bossLabel = BossBox:AddLabel("Boss: 空闲")
local bossCountLabel = BossBox:AddLabel("下个 Boss: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Boss: 进入中") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Boss: 水晶 (" .. #crystals .. " 剩余)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Boss: 攻击手臂 (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Boss: 攻击 Boss（无手臂）") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Boss: 等待（阶段转换）") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("水晶: " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Boss: 进行中（在竞技场）")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("下个 Boss 约 ~%d:%02d 后"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("光明对黑暗")
    LvdBox:AddToggle("se_lvd", { Text = "自动光明对黑暗", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "队伍" })
    local lvdLabel = LvdBox:AddLabel("LvD: 空闲")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "光明对黑暗 已开始 -- " .. fmtT(dur) .. " 剩余", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: 加入队伍中") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " 剩余") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " 环" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: 等待中（由管理员触发，无计时）" or "LvD: 空闲") end)
                task.wait(1)
            end
        end
    end)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("没有可融合的三只同款"); return end
                setStatus("正在融合宠物")
...
ProtBox:AddToggle("se_antiafk", { Text = "防挂机", Default = true })
ProtBox:AddToggle("se_antilag", { Text = "减少卡顿", Default = false, Tooltip = "关闭粒子/阴影/特效、降低画质，并隐藏其他玩家的宠物（客户端）以消除传送卡顿", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("区域")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "守卫穿透", Default = true, Tooltip = "让守卫动物可穿透（客户端），这样它们在你偷取时无法阻挡或推开你", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "锁定时间" })
...
local FG = SuggestTab:AddLeftGroupbox("建议 / 帮助", "message-square")
...
local HelpGroup = SuggestTab:AddRightGroupbox("帮助 / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "加入 Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("已复制 Discord 链接。粘贴到你的浏览器。")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("有建议或发现了bug？\n选择类型，输入消息，然后按发送。", true)

notify("COMBO_WICK 已加载")
