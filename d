...
function Module.updateRadiusCache()
	...
	if currentTime - Module.lastRadiusUpdate < 2 then
		return
	end
	Module.lastRadiusUpdate = currentTime
	
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= Module.player and p.Character then
			local foodRadius = p.Character:FindFirstChild("FoodCollectionRadius", true)
			if foodRadius and (foodRadius:IsA("NumberValue") or foodRadius:IsA("IntValue")) then
				Module.radiusCache[p.UserId] = foodRadius.Value + Module.SAFETY_BUFFER
			else
				Module.radiusCache[p.UserId] = 100 + Module.SAFETY_BUFFER
			end
		end
	end
end
...
function Module.isPlayerNearby(pos)
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= Module.player and p.Character then
			...
			if r then
				local safeDistance = Module.radiusCache[p.UserId] or 130
