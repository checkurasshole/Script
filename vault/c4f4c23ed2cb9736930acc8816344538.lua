...
-- Script ID: c4f4c23ed2cb9736930acc8816344538
-- Updated: 2026-09-14T17:51:39.816Z
...
local Tab = Window:AddTab("Roubar", "shopping-bag")
local Box = Tab:AddLeftGroupbox("Roubar Ovos")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Áreas" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Raridades" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mutações" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Prioridade de Alvo" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Método de Roubo", Tooltip = "Após a pancada da galinha: Instant TP encaixa no ovo (mais rápido); Second Method corre até ele pela pista como o auto-steal antigo (sem teleporte)" })
Box:AddToggle("se_selected",   { Text = "Roubo automático", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Passos automáticos (escolha o que o farm faz)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "Não vender (raridades)" })
Box:AddToggle("se_return",     { Text = "Retornar à base automaticamente", Default = true })
Box:AddButton({ Text = "Sair da Esteira", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Info")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Roubo: Ativo")
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Travado") .. " -> reiniciar por respawn")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: API bloqueada pelo executor (0 servidores)" or "Hop: nenhum servidor disponível, tente de novo")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d servidores, mais vazio %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: todos falharam, tente de novo"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("Nenhum bastão disponível para equipar") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Bastão equipado pelo slot da hotbar " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("As teclas da hotbar não equiparam - mantenha a janela do Roblox em foco e tente de novo") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Movimento")
MoveBox:AddToggle("se_tpwalk", { Text = "Velocidade", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Velocidade de Caminhada", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Voo", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Velocidade de Voo", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Anti Empurrão", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Anti Golpe (repega se cair)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "ESP de Ovos (todos os ovos)", Default = false })
MoveBox:AddButton({ Text = "suicídio", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Reiniciar (Respawn) ao Falhar", Default = true, Tooltip = "Quando um ciclo de roubo trava e te deixa preso no campo, renasce na base (via suicídio) em vez de voltar lentamente -- reinício muito mais rápido" })

local ServerBox = Tab:AddRightGroupbox("Servidor")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Preferência de Hop" })
ServerBox:AddButton({ Text = "Trocar de Servidor", Func = function()
    local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
    task.spawn(function() serverHop(pref) end)
end })
ServerBox:AddButton({ Text = "Reentrar", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "Auto Hop quando cheio", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Trocar com jogadores >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Quando o Auto Hop está ligado e o servidor atinge esse número de jogadores, troca para um novo servidor (usa a Preferência de Hop)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Servidor cheio (" .. #Players:GetPlayers() .. ") -> trocando")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "PAUSADO - FPS baixo (" .. fps .. ")"
            elseif not stealAllowed() then disp = "Pausado (reset de ovos)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "Ativo"
            else disp = "Ocioso" end
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("Esteira não encontrada"); return end
    setStatus("Indo para a esteira")
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("Pausado (reset de ovos)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("Esperando por ovos") end
                return
            end
            ...
            if not fr then setStatus("Nenhum ovo da floresta para preparar"); unlockMove(); return end
            setStatus("Preparar: ovo da floresta")
            ...
            setStatus("Esperando o golpe da galinha...")
            ...
            if not hit then setStatus("Nenhum golpe detectado em 25s"); resetToBase("sem golpe da galinha"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("Falha ao pegar o alvo"); resetToBase("falha ao pegar"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("Ovo caiu (anti-golpe desligado)"); break end
                        setStatus("Ovo caiu -> voltando para pegá-lo")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("falha ao retornar") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
...
local BloomBox = EventsTab:AddLeftGroupbox("Grande Floração")
BloomBox:AddToggle("se_bloom", { Text = "Auto Farmar Bloom", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("Máquina de Fusão")
FuseBox:AddToggle("se_autofuse", { Text = "Auto Fundir Pets", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Incubadora Sakura")
local crysLabel = IncBox:AddLabel("Cristais: 0")
IncBox:AddToggle("se_incubate", { Text = "Auto Incubar", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Auto Depositar Cristais", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Auto Mutar Quando Cheio", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Evento de Boss")
BossBox:AddToggle("se_boss", { Text = "Auto Boss (entrar + bater cristais)", Default = false })
local bossLabel = BossBox:AddLabel("Boss: ocioso")
local bossCountLabel = BossBox:AddLabel("Próximo boss: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Boss: entrando") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Boss: cristais (" .. #crystals .. " restantes)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Boss: atacando braços (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Boss: atacando o boss (sem braço)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Boss: aguardando (mudança de fase)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("Cristais: " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Boss: ATIVO (na arena)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Próximo boss em ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Luz vs Trevas")
    LvdBox:AddToggle("se_lvd", { Text = "Auto Luz vs Trevas", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Equipe" })
    local lvdLabel = LvdBox:AddLabel("LvD: ocioso")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Luz vs Trevas COMEÇOU -- " .. fmtT(dur) .. " restante", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: entrando na equipe") end); task.wait(0.5)
                else
                    ...
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " anéis" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: aguardando (iniciado pela staff, sem cronômetro)" or "LvD: ocioso") end)
                task.wait(1)
            end
        end
    end)
    ...
    while RUNNING do
        local ok = pcall(function()
            ...
            for _, tree in ipairs(CollectionService:GetTagged("SakuraBloomTree")) do
                ...
                if okp and pos then
                    lockMove()
                    setStatus("Bloom: batendo na árvore")
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Bloom: coletando cristal")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("Sem trio para fundir"); return end
                setStatus("Fundindo pets")
...
local ProtBox = SettingsTab:AddLeftGroupbox("Proteção")
ProtBox:AddToggle("se_antiafk", { Text = "Anti AFK", Default = true })
ProtBox:AddToggle("se_antilag", { Text = "Reduzir Lag", Default = false, Tooltip = "Mata partículas/sombras/efeitos, baixa qualidade e OCULTA os pets de outros jogadores (no cliente) para parar o pico de lag ao teleportar", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Noclip de Guardas", Default = true, Tooltip = "Torna os animais guardas atravessáveis (no cliente) para não te bloquearem ou empurrarem enquanto rouba", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "Travar Tempo" })
...
MenuBox:AddButton({ Text = "Descarregar", Func = function()
...
MenuBox:AddLabel("Alternar Menu"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "Alternar Menu" })
...
local FG = SuggestTab:AddLeftGroupbox("Sugestões / Ajuda", "message-square")
    ...
    Func = function()
        ...
        task.spawn(function()
            ...
            if sok then
                ...
                notify("Seu feedback foi enviado. Obrigado!")
            else
...
local HelpGroup = SuggestTab:AddRightGroupbox("Ajuda / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Entrar no Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Link do Discord copiado. Cole no seu navegador.")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("Tem uma sugestão ou encontrou um bug?\nEscolha um tipo, digite sua mensagem e pressione Enviar.", true)

notify("COMBO_WICK carregado")
