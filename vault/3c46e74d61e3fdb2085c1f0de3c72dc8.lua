...
-- Updated: 2026-09-14T17:51:38.628Z
...
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "판매 안 함 (희귀도)" })
...
local function resetToBase(reason)
    ...
    setStatus((reason or "오류") .. " -> 리스폰으로 초기화")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    notify(("Hop: 서버 %d개, 가장 빈 서버 %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
...
MoveBox:AddToggle("se_suicidereset", { Text = "실패 시 리스폰 초기화", Default = true, Tooltip = "훔치기 사이클이 오류로 필드에 갇히면, 천천히 돌아오는 대신 기지에서 리스폰(자살 이용) -- 훨씬 빠른 재시작" })
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not hit then setStatus("25초 동안 피격 없음"); resetToBase("치킨 타격 없음"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("대상 획득 실패"); resetToBase("획득 실패"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                if not delivered and not fpCarrying then resetToBase("복귀 실패") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("빛 대 어둠")
    LvdBox:AddToggle("se_lvd", { Text = "자동 빛 대 어둠", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "팀" })
    local lvdLabel = LvdBox:AddLabel("LvD: 대기")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "빛 대 어둠 시작 -- " .. fmtT(dur) .. " 남음", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: 팀 참가 중") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " 남음") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " 링" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: 대기 중 (운영자 시작, 타이머 없음)" or "LvD: 대기") end)
                task.wait(1)
            end
        end
    end)
