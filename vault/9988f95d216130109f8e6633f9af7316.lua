-- Thai Translation
-- Script ID: 9988f95d216130109f8e6633f9af7316
-- Updated: 2026-09-14T17:51:43.000Z
...
local Tab = Window:AddTab("ขโมย", "shopping-bag")
local Box = Tab:AddLeftGroupbox("ขโมยไข่")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "พื้นที่" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "ความหายาก" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "การกลายพันธุ์" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "ลำดับความสำคัญเป้าหมาย" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "วิธีขโมย", Tooltip = "หลังโดนไก่ตี: Instant TP เด้งไปติดไข่ทันที (เร็วสุด); Second Method วิ่งไปหามันตามเลนเหมือนออโต้สตีลเก่า (ไม่มีเทเลพอร์ต)" })
Box:AddToggle("se_selected",   { Text = "ขโมยอัตโนมัติ", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "ขั้นตอนอัตโนมัติ (เลือกสิ่งที่ฟาร์มทำ)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "ห้ามขาย (ความหายาก)" })
Box:AddToggle("se_return",     { Text = "กลับฐานอัตโนมัติ", Default = true })
Box:AddButton({ Text = "ออกจากลู่วิ่ง", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("ข้อมูล")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("ขโมย: ทำงาน")
InfoBox:AddButton({ Text = "สแกนพื้นที่ใหม่", Func = function()
    local areas = {}
...
local function resetToBase(reason)
    ...
    setStatus((reason or "ค้าง") .. " -> รีเซ็ตด้วยการเกิดใหม่")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: executor บล็อก API (0 เซิร์ฟเวอร์)" or "Hop: ไม่มีเซิร์ฟเวอร์ให้เข้า ลองใหม่")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d เซิร์ฟเวอร์ ว่างสุด %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: ล้มเหลวทั้งหมด ลองใหม่"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("ไม่มีไม้ให้สวม") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("สวมไม้ผ่านช่องฮอตบาร์ " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("ปุ่มฮอตบาร์สวมไม่ได้ - ให้หน้าต่าง Roblox โฟกัสไว้แล้วลองใหม่") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("การเคลื่อนที่")
MoveBox:AddToggle("se_tpwalk", { Text = "ความเร็วเดิน", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "ความเร็วเดิน", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "บิน", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "ความเร็วบิน", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "กันกระแทก", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "กันโดนตี (คว้าใหม่ถ้าหลุด)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "ESP ไข่ (ไข่ทั้งหมด)", Default = false })
MoveBox:AddButton({ Text = "ฆ่าตัวตาย", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "รีเซ็ตด้วยการเกิดใหม่เมื่อพลาด", Default = true, Tooltip = "เมื่อรอบการขโมยค้างและทำให้ติดอยู่กลางสนาม จะเกิดใหม่ที่ฐาน (ผ่านการฆ่าตัวตาย) แทนการค่อยๆ เคลื่อนกลับ -- เริ่มใหม่เร็วกว่ามาก" })

local ServerBox = Tab:AddRightGroupbox("เซิร์ฟเวอร์")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "ตัวเลือกการย้ายเซิร์ฟเวอร์" })
ServerBox:AddButton({ Text = "ย้ายเซิร์ฟเวอร์", Func = function()
    local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
    task.spawn(function() serverHop(pref) end)
end })
ServerBox:AddButton({ Text = "เข้าใหม่", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "ย้ายเซิร์ฟอัตโนมัติเมื่อเต็ม", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "ย้ายเมื่อผู้เล่น >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "เมื่อเปิด Auto Hop และเซิร์ฟเวอร์มีผู้เล่นถึงจำนวนนี้ จะย้ายไปเซิร์ฟใหม่ (ใช้ตัวเลือกการย้าย)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("เซิร์ฟเวอร์เต็ม (" .. #Players:GetPlayers() .. ") -> กำลังย้าย")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "หยุดชั่วคราว - FPS ต่ำ (" .. fps .. ")"
            elseif not stealAllowed() then disp = "หยุดชั่วคราว (รีเซ็ตไข่)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "ทำงาน"
            else disp = "ว่าง" end
            stateLabel:SetText("สถานะ: " .. tostring(disp))
        end)
        task.wait(0.5)
    end
end)
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("ไม่พบลู่วิ่ง"); return end
    setStatus("กำลังไปที่ลู่วิ่ง")
    ...
    onTread = true
    setStatus("อยู่บนลู่วิ่ง (ไม่มีไข่)")
end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("หยุดชั่วคราว (รีเซ็ตไข่)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("รอไข่") end
                return
            end
            ...
            if not fr then setStatus("ไม่มีไข่ป่าให้เตรียม"); unlockMove(); return end
            setStatus("เตรียม: ไข่ป่า")
            ...
            if not hit then setStatus("ไม่พบการโดนตีใน 25 วินาที"); resetToBase("ไม่โดนไก่ตี"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("คว้าเป้าหมายล้มเหลว"); resetToBase("คว้าล้มเหลว"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("ไข่หล่น (กันโดนตีปิด)"); break end
                        setStatus("ไข่หล่น -> กลับไปเก็บ")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("กลับล้มเหลว") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("กำลังกู้คืน...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("อีเวนต์", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("เกรทบลูม")
BloomBox:AddToggle("se_bloom", { Text = "ฟาร์มบลูมอัตโนมัติ", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("เครื่องหลอมรวม")
FuseBox:AddToggle("se_autofuse", { Text = "หลอมรวมสัตว์เลี้ยงอัตโนมัติ", Default = false })
local IncBox = EventsTab:AddRightGroupbox("ตู้ฟักซากุระ")
local crysLabel = IncBox:AddLabel("คริสตัล: 0")
IncBox:AddToggle("se_incubate", { Text = "ฟักอัตโนมัติ", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "ฝากคริสตัลอัตโนมัติ", Default = true })
IncBox:AddToggle("se_automutate", { Text = "กลายพันธุ์อัตโนมัติเมื่อเต็ม", Default = true })

local BossBox = EventsTab:AddRightGroupbox("อีเวนต์บอส")
BossBox:AddToggle("se_boss", { Text = "บอสอัตโนมัติ (เข้า + ตีคริสตัล)", Default = false })
local bossLabel = BossBox:AddLabel("บอส: ว่าง")
local bossCountLabel = BossBox:AddLabel("บอสถัดไป: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("บอส: กำลังเข้า") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("บอส: คริสตัล (" .. #crystals .. " เหลือ)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("บอส: โจมตีแขน (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("บอส: โจมตีบอส (ไม่มีแขน)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("บอส: รอ (เปลี่ยนเฟส)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("คริสตัล: " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("บอส: ทำงาน (ในอารีนา)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("บอสถัดไปใน ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("แสงสว่างปะทะความมืด")
    LvdBox:AddToggle("se_lvd", { Text = "แสงสว่างปะทะความมืดอัตโนมัติ", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "ทีม" })
    local lvdLabel = LvdBox:AddLabel("LvD: ว่าง")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "แสงสว่างปะทะความมืด เริ่มแล้ว -- " .. fmtT(dur) .. " เหลือ", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: กำลังเข้าทีม") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " เหลือ") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " วง" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: กำลังรอ (ทีมงานเริ่ม, ไม่มีตัวจับเวลา)" or "LvD: ว่าง") end)
                task.wait(1)
            end
        end
    end)
        ...
        local ok = pcall(function()
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("บลูม: กำลังเก็บคริสตัล")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("ไม่มีชุดสามชิ้นให้หลอมรวม"); return end
                setStatus("กำลังหลอมรวมสัตว์เลี้ยง")
...
ProtBox:AddToggle("se_antiafk", { Text = "กัน AFK", Default = true })
ProtBox:AddToggle("se_antilag", { Text = "ลดแลค", Default = false, Tooltip = "ปิดพาร์ติเคิล/เงา/เอฟเฟกต์ ลดคุณภาพ และซ่อนสัตว์เลี้ยงของผู้เล่นอื่น (ฝั่งไคลเอนต์) เพื่อหยุดอาการแลกตอนเทเลพอร์ต", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("พื้นที่")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "โนคลิปการ์ด", Default = true, Tooltip = "ทำให้สัตว์การ์ดทะลุผ่านได้ (ฝั่งไคลเอนต์) เพื่อไม่ให้ขวางหรือดันคุณตอนขโมย", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "ล็อกเวลา" })
...
local FB_ERRMAP = {
    ...
    rate_limited = "คุณถึงขีดจำกัด 3 ข้อความแล้ว โปรดลองใหม่ภายหลัง",
    busy = "เซิร์ฟเวอร์ไม่ว่าง โปรดลองใหม่", banned = "คุณไม่ได้รับอนุญาตให้ส่งฟีดแบ็ก",
    discord_error = "ส่งไม่สำเร็จ โปรดลองใหม่", network = "เกิดข้อผิดพลาดเครือข่าย",
    no_request = "executor ของคุณส่งคำขอไม่ได้", bad_response = "เซิร์ฟเวอร์ตอบกลับไม่คาดคิด",
    unreachable = "executor ของคุณเชื่อมต่อเซิร์ฟเวอร์ไม่ได้ (TLS error) โปรดลอง executor อื่น",
}
...
local FG = SuggestTab:AddLeftGroupbox("ข้อเสนอแนะ / ช่วยเหลือ", "message-square")
...
FG:AddButton({
    ...
    Func = function()
        ...
        if #msg > 500 then pcall(function() fbStatus:SetText("ข้อความยาวเกินไป (สูงสุด 500 ตัวอักษร)") end); return end
        local low = msg:lower()
        ...
        task.spawn(function()
            ...
            if sok then
                ...
                pcall(function() fbStatus:SetText(FB_ERRMAP[err] or ("ส่งไม่สำเร็จ: " .. tostring(err))) end)
            end
        end)
    end,
})
FG:AddLabel("สูงสุด 3 ข้อความต่อชั่วโมง ห้ามลิงก์หรือกล่าวถึง", true)

local HelpGroup = SuggestTab:AddRightGroupbox("ช่วยเหลือ / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "เข้าร่วม Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("คัดลอกลิงก์ Discord แล้ว วางในเบราว์เซอร์ของคุณ")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("มีข้อเสนอแนะหรือเจอบั๊ก?\nเลือกประเภท พิมพ์ข้อความ แล้วกดส่ง", true)

notify("โหลด COMBO_WICK แล้ว")
