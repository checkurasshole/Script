...
-- Script ID: 439271281dbbf38e629c3aa61d0a1cfb
-- Updated: 2026-09-14T17:51:43.931Z
...
local Tab = Window:AddTab("Trộm", "shopping-bag")
local Box = Tab:AddLeftGroupbox("Trộm trứng")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Khu vực" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Độ hiếm" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Đột biến" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Ưu tiên mục tiêu" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Cách trộm", Tooltip = "Sau khi bị gà đánh: Instant TP dính ngay vào trứng (nhanh nhất); Second Method chạy tới nó theo làn như auto-steal cũ (không dịch chuyển)" })
Box:AddToggle("se_selected",   { Text = "Tự động trộm", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Bước tự động (chọn việc farm làm)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "Không bán (độ hiếm)" })
Box:AddToggle("se_return",     { Text = "Tự động về căn cứ", Default = true })
Box:AddButton({ Text = "Rời máy chạy", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Thông tin")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Trộm: Đang chạy")
InfoBox:AddButton({ Text = "Quét lại khu vực", Func = function()
    local areas = {}
    ...
    notify(#al .. " khu vực được tìm thấy")
end })
local fps = 0
do
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Lỗi") .. " -> đặt lại bằng hồi sinh")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: executor chặn API (0 máy chủ)" or "Hop: không có máy chủ để vào, thử lại")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d máy chủ, trống nhất %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: tất cả thất bại, thử lại"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("Không có gậy để trang bị") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Đã trang bị gậy qua ô hotbar " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("Phím hotbar không trang bị được - giữ cửa sổ Roblox được focus và thử lại") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Di chuyển")
MoveBox:AddToggle("se_tpwalk", { Text = "Tốc độ đi", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Tốc độ đi bộ", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Bay", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Tốc độ bay", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Chống hất văng", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Chống trúng đòn (nhặt lại nếu rơi)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "ESP trứng (mọi trứng)", Default = false })
MoveBox:AddButton({ Text = "tự sát", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Hồi sinh lại khi lỗi", Default = true, Tooltip = "Khi một chu kỳ trộm bị lỗi và kẹt bạn ngoài đồng, hồi sinh tại căn cứ (bằng tự sát) thay vì di chuyển chậm về -- khởi động lại nhanh hơn nhiều" })

local ServerBox = Tab:AddRightGroupbox("Máy chủ")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Tùy chọn nhảy server" })
ServerBox:AddButton({ Text = "Nhảy máy chủ", Func = function()
    local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
    task.spawn(function() serverHop(pref) end)
end })
ServerBox:AddButton({ Text = "Vào lại", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "Tự nhảy server khi đầy", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Nhảy khi người chơi >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Khi Auto Hop bật và máy chủ đạt số người chơi này, sẽ nhảy sang máy chủ mới (dùng Tùy chọn nhảy server)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Máy chủ đầy (" .. #Players:GetPlayers() .. ") -> đang nhảy")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "TẠM DỪNG - FPS thấp (" .. fps .. ")"
            elseif not stealAllowed() then disp = "Tạm dừng (reset trứng)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "Đang chạy"
            else disp = "Chờ" end
            stateLabel:SetText("Trạng thái: " .. tostring(disp))
        end)
        task.wait(0.5)
    end
end)
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("Không tìm thấy máy chạy"); return end
    setStatus("Đang tới máy chạy")
    ...
    onTread = true
    setStatus("Trên máy chạy (không có trứng)")
end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("Tạm dừng (reset trứng)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("Đang chờ trứng") end
                return
            end
            ...
            if not fr then setStatus("Không có trứng rừng để mồi"); unlockMove(); return end
            setStatus("Mồi: trứng rừng")
            ...
            if not hit then setStatus("Không phát hiện đòn trong 25 giây"); resetToBase("không bị gà đánh"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("Lấy mục tiêu thất bại"); resetToBase("lấy thất bại"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("Rơi trứng (chống trúng đòn tắt)"); break end
                        setStatus("Rơi trứng -> quay lại lấy")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("về thất bại") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("Đang khôi phục...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("Sự kiện", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("Great Bloom")
BloomBox:AddToggle("se_bloom", { Text = "Tự động farm Bloom", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("Máy hợp nhất")
FuseBox:AddToggle("se_autofuse", { Text = "Tự động hợp nhất thú cưng", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Lò ấp Sakura")
local crysLabel = IncBox:AddLabel("Pha lê: 0")
IncBox:AddToggle("se_incubate", { Text = "Tự động ấp", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Tự động nạp pha lê", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Tự động đột biến khi đầy", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Sự kiện Trùm")
BossBox:AddToggle("se_boss", { Text = "Tự động Trùm (vào + đánh pha lê)", Default = false })
local bossLabel = BossBox:AddLabel("Trùm: chờ")
local bossCountLabel = BossBox:AddLabel("Trùm kế: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Trùm: đang vào") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Trùm: pha lê (" .. #crystals .. " còn lại)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Trùm: đánh tay (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Trùm: đánh trùm (không còn tay)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Trùm: chờ (đổi pha)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("Pha lê: " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Trùm: HOẠT ĐỘNG (trong đấu trường)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Trùm kế sau ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Ánh Sáng vs Bóng Tối")
    LvdBox:AddToggle("se_lvd", { Text = "Tự động Ánh Sáng vs Bóng Tối", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Đội" })
    local lvdLabel = LvdBox:AddLabel("LvD: chờ")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Ánh Sáng vs Bóng Tối ĐÃ BẮT ĐẦU -- " .. fmtT(dur) .. " còn lại", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: đang vào đội") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " còn lại") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " vòng" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: đang chờ (staff kích hoạt, không hẹn giờ)" or "LvD: chờ") end)
                task.wait(1)
            end
        end
    end)
        ...
        local ok = pcall(function()
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Bloom: đang thu pha lê")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("Không có bộ ba để hợp nhất"); return end
                setStatus("Đang hợp nhất thú cưng")
...
local ProtBox = SettingsTab:AddLeftGroupbox("Bảo vệ")
ProtBox:AddToggle("se_antiafk", { Text = "Chống AFK", Default = true })
ProtBox:AddToggle("se_antilag", { Text = "Giảm lag", Default = false, Tooltip = "Tắt hạt/bóng/hiệu ứng, chất lượng thấp, và ẨN thú cưng của người chơi khác (phía client) để chặn giật lag khi dịch chuyển", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Khu vực")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Noclip lính gác", Default = true, Tooltip = "Làm cho thú lính gác đi xuyên qua được (phía client) để chúng không chặn hay đẩy bạn khi trộm", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "Khóa thời gian" })
...
MenuBox:AddButton({ Text = "Gỡ tải", Func = function()
...
MenuBox:AddLabel("Bật/Tắt Menu"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "Bật/Tắt Menu" })
...
local FB_ERRMAP = {
    empty = "Tin nhắn trống.", no_links = "Không được phép gửi link.",
    too_long = "Tin nhắn quá dài (tối đa 500 ký tự).",
    rate_limited = "Bạn đã đạt giới hạn 3 tin nhắn. Vui lòng thử lại sau.",
    ...
    no_request = "Executor của bạn không gửi được yêu cầu.", bad_response = "Máy chủ trả về phản hồi bất ngờ.",
    unreachable = "Executor của bạn không kết nối được máy chủ (lỗi TLS). Hãy thử executor khác.",
}
...
local SuggestTab = Window:AddTab("Góp ý", "message-square")
local FG = SuggestTab:AddLeftGroupbox("Góp ý / Trợ giúp", "message-square")
...
FG:AddButton({
    ...
    Func = function()
        ...
        if now - lastFeedback < FEEDBACK_COOLDOWN then
            pcall(function() fbStatus:SetText(string.format("Vui lòng đợi %d giây trước khi gửi lại.", math.ceil(FEEDBACK_COOLDOWN - (now - lastFeedback)))) end)
            return
        end
        ...
        if msg == "" then pcall(function() fbStatus:SetText("Tin nhắn trống.") end); return end
        if #msg > 500 then pcall(function() fbStatus:SetText("Tin nhắn quá dài (tối đa 500 ký tự).") end); return end
        local low = msg:lower()
        if low:find("http", 1, true) or low:find("discord.gg", 1, true) or low:find("www.", 1, true) or low:find(".gg/", 1, true) then
            pcall(function() fbStatus:SetText("Không được phép gửi link.") end); return
        end
        pcall(function() fbStatus:SetText("Đang gửi...") end)
        task.spawn(function()
            local sok, err = sendFeedback(ftype, msg)
            if sok then
                lastFeedback = tick()
                pcall(function() fbStatus:SetText("Đã gửi. Cảm ơn!") end)
                pcall(function() Opt.FeedbackMsg:SetValue("") end)
                notify("Phản hồi của bạn đã được gửi. Cảm ơn!")
            else
                pcall(function() fbStatus:SetText(FB_ERRMAP[err] or ("Gửi thất bại: " .. tostring(err))) end)
            end
        end)
    end,
})
FG:AddLabel("Tối đa 3 tin nhắn mỗi giờ. Không link hoặc nhắc tên.", true)

local HelpGroup = SuggestTab:AddRightGroupbox("Trợ giúp / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Tham gia Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Đã sao chép link Discord. Dán vào trình duyệt của bạn.")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("Có góp ý hoặc phát hiện lỗi?\nChọn loại, nhập tin nhắn và nhấn Gửi.", true)

notify("COMBO_WICK đã tải")
