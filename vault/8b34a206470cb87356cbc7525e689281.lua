...
-- Script ID: 8b34a206470cb87356cbc7525e689281
-- Migrated: 2025-09-11T14:25:38.813Z
-- Auto-migrated from encrypted storage to GitHub

loadstring(game:HttpGet("__URL_230d25d33b989140__"))()
...
local Fluent = loadstring(game:HttpGet("__URL_d23c7b1734f6ea81__"))()
...
Tab:AddParagraph({
    Title = "God Mode Status God Mode is automatically enabled on script load",
    Content = "Works best in Single Player"
})
...
-- Enhanced Auto Objective Variables
local ObjectiveAutomationActive = false
local AutoTeleportWasEnabled = false
local AutoTeleportToggle
local firing = false -- prevent spam firing for auto-fire functionality
...
-- Function to get nearest prompt for auto-fire
...
-- Enhanced activatePrompt function with auto-fire integration
local function activatePrompt(prompt, targetObject, maxRetries)
    ...
    local function attemptActivation()
        ...
        -- Enhanced auto-fire loop for radio objectives
        if not wasTriggered and (targetObject.Name == "jarst_radio" or targetObject.Name == "RadioObjective") then
            print("Starting enhanced auto-fire sequence for radio objective...")
            ...
            while not wasTriggered and autoFireAttempts < maxAutoFireAttempts do
                ...
                if not firing then
                    ...
                    -- Try to get the nearest prompt (should be our target)
                    local nearestPrompt = getNearestPrompt()
                    if nearestPrompt and nearestPrompt == prompt then
                        print("Auto-firing nearest prompt (attempt " .. autoFireAttempts .. ")")
                        pcall(function()
                        ...
                        -- Additional firing methods for radio objectives
                        pcall(function()
                        ...
                        -- Try multiple firing approaches
                        for i = 1, 3 do
                            pcall(function()
                                fireproximityprompt(nearestPrompt)
                            end)
                ...
                if wasTriggered then
                    print("Auto-fire successful after " .. autoFireAttempts .. " attempts!")
                    break
                end
            end
        end
                ...
                if not descendant.Parent then
                    ...
                    pcall(function()
                        ...
                        if heliObj and heliObj.Parent then
                            print("Found HeliObjective via specific path!")
                        else
                            heliObj = nil
                        end
                    end)
                    
                    -- Fall back to searching all descendants if specific path fails
                    if not heliObj then
                    ...
                    if heliObj then
                        print("Found HeliObjective! Starting helicopter automation...")
                        
                        if TeleportActive and AutoTeleportToggle then
                            print("Disabling Auto Teleport for helicopter objective...")
                            AutoTeleportWasEnabled = true
                            AutoTeleportToggle:SetValue(false)
                        end
                        
                        print("Teleporting to HeliObjective...")
                        teleportTo(heliObj)
                        task.wait(0.3)
                        
                        local heliPrompt = nil
                        
                        -- Try specific path for prompt first
                        pcall(function()
                            ...
                            if heliPrompt and heliPrompt:IsA("ProximityPrompt") then
                                print("Found HeliObjective ProximityPrompt via specific path!")
                            else
                                heliPrompt = nil
                            end
                        end)
                        
                        -- Fall back to searching if specific path fails
                        ...
                        if heliPrompt then
                            print("Found helicopter ProximityPrompt! Using enhanced activation...")
                            activatePrompt(heliPrompt, heliObj, 20)
                        else
                            print("No proximity prompt found in HeliObjective, trying ClickDetector...")
                            local clickDetector = heliObj:FindFirstChildOfClass("ClickDetector")
                            if clickDetector then
                                pcall(function()
                                    fireclickdetector(clickDetector)
                                end)
                                print("Fired ClickDetector for HeliObjective")
                            else
                                print("No ClickDetector found either!")
                            end
                        end
                    else
            ...
            descendant.AncestryChanged:Connect(function()
                if not descendant.Parent then
                    ...
                    pcall(function()
                        heliObj = workspace.School.Rooms.RooftopBoss.HeliObjective
                        if heliObj and heliObj.Parent then
                            print("Found existing HeliObjective via specific path!")
                        else
                            heliObj = nil
                        end
                    end)
                    ...
                    if heliObj then
                        if TeleportActive and AutoTeleportToggle then
                            print("Disabling Auto Teleport for existing helicopter objective...")
                            AutoTeleportWasEnabled = true
                            AutoTeleportToggle:SetValue(false)
                        end
                        
                        print("Teleporting to existing HeliObjective...")
                        teleportTo(heliObj)
                        task.wait(0.3)
                        ...
                        pcall(function()
                            heliPrompt = workspace.School.Rooms.RooftopBoss.HeliObjective.ProximityPrompt
                            if heliPrompt and heliPrompt:IsA("ProximityPrompt") then
                                print("Found existing HeliObjective ProximityPrompt via specific path!")
                            else
                                heliPrompt = nil
                            end
                        end)
                        ...
                        if heliPrompt then
                            print("Found existing helicopter ProximityPrompt! Using enhanced activation...")
                            activatePrompt(heliPrompt, heliObj, 20)
                        else
                            print("No proximity prompt found in existing HeliObjective")
                            local clickDetector = heliObj:FindFirstChildOfClass("ClickDetector")
                            if clickDetector then
                                pcall(function()
                                    fireclickdetector(clickDetector)
...
local function GetBestMobTarget()
    ...
    -- Function to check if an entity is valid and get its parts
    ...
    -- First check for direct entities in the Entities folder (old structure)
    for _, entity in pairs(entitiesFolder:GetChildren()) do
        processEntity(entity)
    end
    
    -- Then check for entities in zombie type folders (new structure)
    for _, folder in pairs(entitiesFolder:GetChildren()) do
        if folder:IsA("Folder") or folder:IsA("Model") then
            -- Check if this is a zombie type folder (like "Zombie")
            if folder.Name ~= "Highlight" then
                for _, numberedFolder in pairs(folder:GetChildren()) do
                    -- Check numbered folders like "1", "2", etc.
...
ObjectiveSection:AddToggle("ObjectiveAutomation", {
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            checkExisting()
            print("Enhanced Auto Objective system loaded! Monitoring for objectives...")
        else
