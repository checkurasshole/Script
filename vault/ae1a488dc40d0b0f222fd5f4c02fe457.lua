...
-- Script ID: ae1a488dc40d0b0f222fd5f4c02fe457
-- Updated: 2026-09-14T17:51:37.630Z
...
local Tab = Window:AddTab("Curi", "shopping-bag")
local Box = Tab:AddLeftGroupbox("Curi Telur")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Area" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Kelangkaan" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mutasi" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Prioritas Target" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Metode Curi", Tooltip = "Setelah pukulan ayam: Instant TP langsung menempel ke telur (tercepat); Second Method berlari ke sana lewat lane seperti auto-steal lama (tanpa teleport)" })
Box:AddToggle("se_selected",   { Text = "Curi otomatis", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Langkah otomatis (pilih aksi farm)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "Jangan jual (kelangkaan)" })
Box:AddToggle("se_return",     { Text = "Auto kembali ke markas", Default = true })
Box:AddButton({ Text = "Keluar dari Treadmill", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Info")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Curi: Aktif")
InfoBox:AddButton({ Text = "Pindai ulang area", Func = function()
    local areas = {}
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Error") .. " -> reset via respawn")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: API diblokir executor (0 server)" or "Hop: tak ada server, coba lagi")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d server, terkosong %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: semua gagal, coba lagi"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("Tak ada tongkat untuk dipakai") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Tongkat dipasang lewat slot hotbar " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("Tombol hotbar gagal memasang - jaga jendela Roblox tetap fokus lalu coba lagi") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Gerakan")
MoveBox:AddToggle("se_tpwalk", { Text = "Kecepatan Jalan", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Kecepatan Berjalan", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Terbang", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Kecepatan Terbang", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Anti Knockback", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Anti Pukul (ambil lagi jika jatuh)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "ESP Telur (semua telur)", Default = false })
MoveBox:AddButton({ Text = "bunuh diri", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Reset Respawn saat Gagal", Default = true, Tooltip = "Saat siklus mencuri error dan membuatmu terjebak di lapangan, respawn di markas (lewat bunuh diri) daripada kembali pelan-pelan -- restart jauh lebih cepat" })

local ServerBox = Tab:AddRightGroupbox("Server")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Preferensi Hop" })
ServerBox:AddButton({ Text = "Pindah Server", Func = function()
    local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
    task.spawn(function() serverHop(pref) end)
end })
ServerBox:AddButton({ Text = "Gabung Ulang", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "Auto Hop saat penuh", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Hop saat pemain >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Saat Auto Hop nyala dan server mencapai jumlah pemain ini, pindah ke server baru (memakai Preferensi Hop)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Server penuh (" .. #Players:GetPlayers() .. ") -> berpindah")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "DIJEDA - FPS rendah (" .. fps .. ")"
            elseif not stealAllowed() then disp = "Dijeda (reset telur)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "Aktif"
            else disp = "Diam" end
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("Treadmill tak ditemukan"); return end
    setStatus("Menuju treadmill")
    ...
    onTread = true
    setStatus("Di treadmill (tak ada telur)")
end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("Dijeda (reset telur)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("Menunggu telur") end
                return
            end
            ...
            if not fr then setStatus("Tak ada telur hutan untuk disiapkan"); unlockMove(); return end
            setStatus("Siapkan: telur hutan")
            ...
            setStatus("Menunggu pukulan ayam...")
            ...
            if not hit then setStatus("Tak ada pukulan dalam 25 detik"); resetToBase("tak kena pukul ayam"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("Gagal mengambil target"); resetToBase("gagal ambil"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("Telur jatuh (anti-pukul mati)"); break end
                        setStatus("Telur jatuh -> kembali mengambil")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("gagal kembali") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("Memulihkan...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("Event", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("Great Bloom")
BloomBox:AddToggle("se_bloom", { Text = "Auto Farm Bloom", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("Mesin Fusi")
FuseBox:AddToggle("se_autofuse", { Text = "Auto Fusi Peliharaan", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Inkubator Sakura")
local crysLabel = IncBox:AddLabel("Kristal: 0")
IncBox:AddToggle("se_incubate", { Text = "Auto Inkubasi", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Auto Setor Kristal", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Auto Mutasi Saat Penuh", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Event Bos")
BossBox:AddToggle("se_boss", { Text = "Auto Bos (masuk + pukul kristal)", Default = false })
local bossLabel = BossBox:AddLabel("Bos: diam")
local bossCountLabel = BossBox:AddLabel("Bos berikutnya: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Bos: masuk") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Bos: kristal (" .. #crystals .. " tersisa)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Bos: menyerang lengan (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Bos: menyerang bos (tak ada lengan)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Bos: menunggu (ganti fase)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Bos: AKTIF (di arena)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Bos berikutnya dalam ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Terang vs Gelap")
    LvdBox:AddToggle("se_lvd", { Text = "Auto Terang vs Gelap", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Tim" })
    local lvdLabel = LvdBox:AddLabel("LvD: diam")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Terang vs Gelap DIMULAI -- " .. fmtT(dur) .. " tersisa", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: bergabung tim") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " tersisa") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " cincin" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: menunggu (dipicu staff, tanpa timer)" or "LvD: diam") end)
                task.wait(1)
            end
        end
    end)
        ...
        local ok = pcall(function()
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Bloom: mengumpulkan kristal")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("Tak ada tiga sejenis untuk difusi"); return end
                setStatus("Memfusi peliharaan")
...
ProtBox:AddToggle("se_antilag", { Text = "Kurangi Lag", Default = false, Tooltip = "Mematikan partikel/bayangan/efek, kualitas rendah, dan MENYEMBUNYIKAN peliharaan pemain lain (sisi klien) untuk menghentikan lonjakan lag saat teleport", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Area")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Noclip Penjaga", Default = true, Tooltip = "Membuat hewan penjaga bisa ditembus (sisi klien) agar tak menghalangi atau mendorongmu saat mencuri", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "Kunci Waktu" })
...
local FB_ERRMAP = {
    ...
    rate_limited = "Kamu mencapai batas 3 pesan. Coba lagi nanti.",
    busy = "Server sibuk. Coba lagi.", banned = "Kamu tidak diizinkan mengirim masukan.",
    discord_error = "Gagal mengirim. Coba lagi.", network = "Kesalahan jaringan.",
    no_request = "Executor-mu tidak bisa mengirim permintaan.", bad_response = "Server mengembalikan respons tak terduga.",
    unreachable = "Executor-mu tak bisa menjangkau server (TLS error). Coba executor lain.",
}
...
local FG = SuggestTab:AddLeftGroupbox("Saran / Bantuan", "message-square")
FG:AddDropdown("FeedbackType", { Text = "Tipe", Values = { "Suggestion", "Bug", "Help" }, Default = "Suggestion", Callback = function() end })
FG:AddInput("FeedbackMsg", { Default = "", Numeric = false, Finished = false, ClearTextOnFocus = false, Text = "Pesan", Placeholder = "Ketik pesanmu (maksimal 500 karakter, tanpa tautan)" })
...
FG:AddButton({
    ...
    Func = function()
        ...
        if #msg > 500 then pcall(function() fbStatus:SetText("Pesan terlalu panjang (maksimal 500 karakter).") end); return end
        local low = msg:lower()
        ...
        task.spawn(function()
            ...
            if sok then
                ...
                pcall(function() fbStatus:SetText(FB_ERRMAP[err] or ("Gagal mengirim: " .. tostring(err))) end)
            end
        end)
    end,
})
...
local HelpGroup = SuggestTab:AddRightGroupbox("Bantuan / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Gabung Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Tautan Discord disalin. Tempel di browser-mu.")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("Punya saran atau menemukan bug?\nPilih tipe, ketik pesan, lalu tekan Kirim.", true)

notify("COMBO_WICK dimuat")
