-- Script_1754747754660
-- Script ID: e45aeb182071baece3470bdbd5dc0501
-- Migrated: 2025-09-11T12:58:18.342Z
-- Auto-migrated from encrypted storage to GitHub

for _, url in ipairs{
    "__URL_4eb07e776d43513e__",
    "__URL_1069da92b9f07a5f__"
} do
    local s = loadstring(game:HttpGet(url))
    if s then s() end
end

local Rayfield = loadstring(game:HttpGet('__URL_4ab427badc862118__'))()
local Window = Rayfield:CreateWindow({
    Name = "COMBO_WICK-KI Lutte contre ...",
    Icon = 12345678901,
...
local RefreshHitboxButton = MainTab:CreateButton({
    Name = "Rafraîchir Hitbox",
    Callback = function()
        refreshHitbox()
        Rayfield:Notify({
            Title = "Hitbox rafraîchi",
            Content = "Toutes les boîtes à succès ont été réappliquées !",
            Duration = 2,
        })
    end,
})

local VisualsSection = VisualsTab:CreateSection("Éléments visuels")
...
local TeleportSection = TeleportTab:CreateSection("Contrôles de téléportation")

local BringPlayersToggle = TeleportTab:CreateToggle({
   Name = "Apportez les joueurs",
   CurrentValue = false,
...
local AimbotSection = AimbotTab:CreateSection("Paramètres Auto Aimbot")
...
local AutoAimDistanceSlider = AimbotTab:CreateSlider({
   Name = "Distance maximale d'objectif",
   Range = {50, 1000},
   Increment = 10,
   CurrentValue = 300,
...
local UnlockTargetButton = AimbotTab:CreateButton({
   Name = "Forcer le déverrouillage de...",
   Callback = function()
       unlockTarget()
   end,
})

local TargetStatusSection = AimbotTab:CreateSection("État de la cible")
...
Rayfield:Notify({
   Title = "ESP, Aimbot et Hitbox chargés",
   Content = "Les systèmes ESP, ajutage et hitbox améliorés avec fonctionnalité de téléport...",
   Duration = 3,
   Image = "eye"
})