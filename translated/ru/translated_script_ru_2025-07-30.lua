...
-- Main Tab
local MainTab = Window:CreateTab("Управление автомобилями", "car")
...
local UpgradeToggle = MainTab:CreateToggle({
   Name = "Модернизация автомобилей и ...",
   CurrentValue = false,
   Flag = "UpgradeToggle",
   Callback = function(Value)
...
-- Upgrades Tab
local UpgradesTab = Window:CreateTab("Модернизация", "trending-up")
...
local FloorToggle = UpgradesTab:CreateToggle({
   Name = "Скидка по нижнему пределу",
   CurrentValue = false,
   Flag = "FloorToggle",
   Callback = function(Value)
