-- German Translation
-- Script ID: a1659bae26af5175530e7df011831d25
-- Updated: 2026-09-14T17:51:36.652Z
...
local Box = Tab:AddLeftGroupbox("Eier stehlen")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Bereiche" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Seltenheiten" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mutationen" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Zielpriorität" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Stehl-Methode", Tooltip = "Nach dem Huhn-Treffer: Instant TP schnappt aufs Ei (am schnellsten); Second Method läuft über die Bahn dorthin wie der alte Auto-Steal (kein Teleport)" })
Box:AddToggle("se_selected",   { Text = "Auto-Stehlen", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Auto-Schritte (wähle, was der Farm macht)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "Nicht verkaufen (Seltenheiten)" })
Box:AddToggle("se_return",     { Text = "Auto-Rückkehr zur Basis", Default = true })
Box:AddButton({ Text = "Laufband verlassen", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Info")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Stehlen: Aktiv")
InfoBox:AddButton({ Text = "Bereiche neu scannen", Func = function()
    local areas = {}
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Fehlerhaft") .. " -> Respawn-Reset")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: API vom Executor blockiert (0 Server)" or "Hop: kein beitretbarer Server, erneut versuchen")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d Server, leerster %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: alle fehlgeschlagen, erneut versuchen"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("Kein Schläger zum Ausrüsten verfügbar") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Schläger über Hotbar-Slot ausgerüstet " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("Hotbar-Tasten haben nicht ausgerüstet - Roblox-Fenster fokussiert lassen und erneut versuchen") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Bewegung")
MoveBox:AddToggle("se_tpwalk", { Text = "Gehtempo", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Gehgeschwindigkeit", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Flug", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Fluggeschwindigkeit", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Anti-Rückstoß", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Anti-Treffer (neu greifen, wenn fallengelassen)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "Ei-ESP (alle Eier)", Default = false })
MoveBox:AddButton({ Text = "Selbstmord", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Respawn-Reset bei Fehler", Default = true, Tooltip = "Wenn ein Steal-Zyklus hängt und dich im Feld stranden lässt, respawne an der Basis (per Selbstmord) statt langsam zurückzugleiten -- viel schnellerer Neustart" })

local ServerBox = Tab:AddRightGroupbox("Server")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Hop-Präferenz" })
...
ServerBox:AddToggle("se_autohop", { Text = "Auto-Hop wenn voll", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Hop bei Spielern >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Wenn Auto-Hop an ist und der Server so viele Spieler erreicht, wird zu einem neuen Server gehoppt (nutzt Hop-Präferenz)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Server voll (" .. #Players:GetPlayers() .. ") -> wechsle")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "PAUSIERT - niedrige FPS (" .. fps .. ")"
            elseif not stealAllowed() then disp = "Pausiert (Ei-Reset)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "Aktiv"
            else disp = "Inaktiv" end
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("Laufband nicht gefunden"); return end
    setStatus("Auf dem Weg zum Laufband")
    ...
    onTread = true
    setStatus("Auf dem Laufband (keine Eier)")
end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("Pausiert (Ei-Reset)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("Warte auf Eier") end
                return
            end
            ...
            if not fr then setStatus("Kein Wald-Ei zum Vorbereiten"); unlockMove(); return end
            setStatus("Vorbereiten: Wald-Ei")
            ...
            setStatus("Warte auf Huhn-Treffer...")
            ...
            if not hit then setStatus("Kein Treffer in 25s erkannt"); resetToBase("kein Hühner-Treffer"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("Ziel greifen fehlgeschlagen"); resetToBase("Greifen fehlgeschlagen"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("Ei fallengelassen (Anti-Treffer aus)"); break end
                        setStatus("Ei fallengelassen -> hole es zurück")
                        local gg = os.clock()
                        ...
                        setStatus("Kehre zur Basis zurück")
                ...
                if not delivered and not fpCarrying then resetToBase("Rückkehr fehlgeschlagen") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("Wiederherstellung...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("Events", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("Große Blüte")
BloomBox:AddToggle("se_bloom", { Text = "Auto-Farm Blüte", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("Fusionsmaschine")
FuseBox:AddToggle("se_autofuse", { Text = "Auto-Pets fusionieren", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Sakura-Inkubator")
local crysLabel = IncBox:AddLabel("Kristalle: 0")
IncBox:AddToggle("se_incubate", { Text = "Auto-Inkubieren", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Auto-Kristalle einzahlen", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Auto-Mutation wenn voll", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Boss-Event")
BossBox:AddToggle("se_boss", { Text = "Auto-Boss (betreten + Kristalle treffen)", Default = false })
local bossLabel = BossBox:AddLabel("Boss: inaktiv")
local bossCountLabel = BossBox:AddLabel("Nächster Boss: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Boss: betrete") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Boss: Kristalle (" .. #crystals .. " übrig)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Boss: greife Arme an (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Boss: greife Boss an (kein Arm)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Boss: warte (Phasenwechsel)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("Kristalle: " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Boss: AKTIV (in der Arena)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Nächster Boss in ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Licht gegen Dunkel")
    LvdBox:AddToggle("se_lvd", { Text = "Auto Licht gegen Dunkel", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Team" })
    local lvdLabel = LvdBox:AddLabel("LvD: inaktiv")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Licht gegen Dunkel GESTARTET -- " .. fmtT(dur) .. " übrig", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: trete Team bei") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " übrig") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " Ringe" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: warte (vom Staff gestartet, kein Timer)" or "LvD: inaktiv") end)
                task.wait(1)
            end
        end
    end)
        ...
        local ok = pcall(function()
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Blüte: sammle Kristall")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("Kein Dreier zum Fusionieren"); return end
                setStatus("Fusioniere Pets")
...
ProtBox:AddToggle("se_antilag", { Text = "Lag reduzieren", Default = false, Tooltip = "Entfernt Partikel/Schatten/Effekte, niedrige Qualität und VERSTECKT die Pets anderer Spieler (clientseitig), um den Teleport-Lag-Spike zu stoppen", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Bereiche")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Wächter-Noclip", Default = true, Tooltip = "Macht Wächter-Tiere durchlässig (clientseitig), damit sie dich beim Stehlen nicht blockieren oder wegschubsen", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "Zeit sperren" })
...
local FB_ERRMAP = {
    ...
    rate_limited = "Du hast das Limit von 3 Nachrichten erreicht. Bitte später erneut versuchen.",
    busy = "Der Server ist ausgelastet. Bitte erneut versuchen.", banned = "Du darfst kein Feedback senden.",
    discord_error = "Senden fehlgeschlagen. Bitte erneut versuchen.", network = "Netzwerkfehler.",
    no_request = "Dein Executor kann keine Anfragen senden.", bad_response = "Der Server hat eine unerwartete Antwort zurückgegeben.",
    unreachable = "Dein Executor kann den Server nicht erreichen (TLS-Fehler). Bitte einen anderen Executor versuchen.",
}
...
local FG = SuggestTab:AddLeftGroupbox("Vorschläge / Hilfe", "message-square")
...
FG:AddButton({
    ...
    Func = function()
        ...
        if #msg > 500 then pcall(function() fbStatus:SetText("Die Nachricht ist zu lang (maximal 500 Zeichen).") end); return end
        local low = msg:lower()
        ...
        task.spawn(function()
            ...
            if sok then
                ...
                pcall(function() fbStatus:SetText(FB_ERRMAP[err] or ("Senden fehlgeschlagen: " .. tostring(err))) end)
            end
        end)
    end,
})
...
local HelpGroup = SuggestTab:AddRightGroupbox("Hilfe / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Discord beitreten",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Discord-Link kopiert. Füge ihn in deinen Browser ein.")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("Hast du einen Vorschlag oder einen Bug gefunden?\nWähle einen Typ, tippe deine Nachricht und drücke Senden.", true)

notify("COMBO_WICK geladen")
