...
-- Script ID: fa780a68f566268224209d468242e3cf
-- Created: 2026-01-03T00:53:13.463Z
-- Source: External API
...
if not versionValid then
    ...
    return {
        success = false,
        error = "falha_na_verificação_da_versão",
...
if VaultModule.compareVersions(VaultModule.CONFIG.VERSION, '1.3.2') <= 0 then
    Library:Notify({
        ...
        Description = 'Esta versão não é mais suportada. Por favor, atualize para a v' .. VaultModule.CONFIG.MIN_VERSION .. '+',
        Time = 10,
    })
    return {
        success = false,
...
if not whitelisted or errorType == "acesso_negado" or errorType == "deteção_de_alteração" or errorType == "chave_expirada" then
    ...
    local MainGroup = Tabs.Validação:AddLeftGroupbox("Insira sua Chave de API")
    ...
    local LinksGroup = Tabs.Informações:AddLeftGroupbox("Links Rápidos")
