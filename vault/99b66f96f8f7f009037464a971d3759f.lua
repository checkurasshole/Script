-- Russian Translation
-- Script ID: 99b66f96f8f7f009037464a971d3759f
-- Updated: 2026-09-14T17:51:40.774Z
...
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Метод кражи", Tooltip = "После удара курицы: Instant TP мгновенно переносит на яйцо (быстрее всего); Second Method бежит к нему по полосе, как старый авто-угон (без телепорта)" })
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Сбой") .. " -> сброс через респавн")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    notify(("Hop: серверов %d, самый пустой %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
...
MoveBox:AddSlider("se_walk",   { Text = "Скорость ходьбы", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
...
MoveBox:AddToggle("se_suicidereset", { Text = "Респавн при сбое", Default = true, Tooltip = "Когда цикл кражи сбоит и оставляет тебя в поле, возрождает на базе (через самоубийство) вместо медленного возврата -- намного быстрее перезапуск" })
...
ServerBox:AddToggle("se_autohop", { Text = "Авто-переход при заполнении", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Переход при игроках >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Когда Авто-переход включён и на сервере столько игроков, переходит на новый сервер (использует настройку перехода)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Сервер полон (" .. #Players:GetPlayers() .. ") -> переход")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not hit then setStatus("Удар не обнаружен за 25с"); resetToBase("нет удара курицы"); unlockMove(); return end
            ...
            if (Opt.se_stealmethod and Opt.se_stealmethod.Value) == "Second Method" then
                clearRagdoll(); setStatus("Иду к яйцу (полоса)")
            ...
            if not haveIt() then setStatus("Не удалось схватить цель"); resetToBase("не удалось схватить"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                if not delivered and not fpCarrying then resetToBase("не удалось вернуться") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Свет против Тьмы")
    LvdBox:AddToggle("se_lvd", { Text = "Авто Свет против Тьмы", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Команда" })
    local lvdLabel = LvdBox:AddLabel("LvD: ожидание")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Свет против Тьмы НАЧАЛСЯ -- " .. fmtT(dur) .. " осталось", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: вход в команду") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " осталось") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " колец" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: ожидание (запуск стаффом, без таймера)" or "LvD: ожидание") end)
                task.wait(1)
            end
        end
    end)
...
ProtBox:AddToggle("se_antilag", { Text = "Снизить лаги", Default = false, Tooltip = "Убирает частицы/тени/эффекты, низкое качество и СКРЫВАЕТ питомцев других игроков (на клиенте), чтобы убрать лаг-спайк при телепорте", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Зоны")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Ноклип охраны", Default = true, Tooltip = "Делает животных-охранников проходимыми (на клиенте), чтобы они не блокировали и не толкали вас во время кражи", Callback = function(v) setGuardNoclip(v) end })
