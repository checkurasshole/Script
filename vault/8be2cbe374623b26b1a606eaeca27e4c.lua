...
-- Script ID: 8be2cbe374623b26b1a606eaeca27e4c
-- Updated: 2025-10-01T20:24:42.462Z
...
local function handleBuildStatus(status, data)
    ...
    elseif status == "cancelled" then
        WindUI:Notify({
            Title = "อาคารถูกยกเลิก",
            Content = "หยุดที่" .. data.wallNumber,
            Icon = "x-circle",
            Duration = 3
        })
    elseif status == "placement_failed" then
        WindUI:Notify({
            Title = "⚠️ การวางตำแหน่งล้มเหลว",
            Content = "ผนัง" .. data.wallNumber,
            Icon = "alert-triangle",
            Duration = 10
        })
    elseif status == "walls_not_appearing" then
        WindUI:Notify({
            Title = "⚠️ ผนังไม่ปรากฏ",
            Content = "",
            Icon = "alert-triangle",
            Duration = 10
        })
...
local MainTab = Window:Tab({ Title = "ภารกิจหลัก", Icon = "zap" })
local DiamondFarmTab = Window:Tab({ Title = "ไดมอนด์ฟาร์ม", Icon = "gem" })
local CollectorTab = Window:Tab({ Title = "นักสะสมไอเท็ม", Icon = "package" })
local PivotTab = Window:Tab({ Title = "MainFire", Icon = "move" })
local CombatTab = Window:Tab({ Title = "ระบบต่อสู้", Icon = "sword" })
local CookingTab = Window:Tab({ Title = "การทำอาหารและอาหาร", Icon = "flame" })
local VisualTab = Window:Tab({ Title = "เครื่องมือภาพ", Icon = "eye" })

-- WINDUI TABS (BELOW)
local LogWallTab = Window:Tab({ Title = "ผนังปูมบันทึก", Icon = "shield" })
local PlantHouseTab = Window:Tab({ Title = "พืช/บ้าน", Icon = "home" })
local ConfigTab = Window:Tab({ Title = "สถานการณ์", Icon = "save" })
...
diamondFarmToggle = DiamondFarmTab:Toggle({  -- Store the toggle reference
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            if success then
                WindUI:Notify({
                    Title = "โหลดไดมอนด์ฟาร์มแล้ว",
                    Content = "เปิดใช้งานสคริปต์เรียบร้อยแล้ว",
                    Icon = "gem",
                    Duration = 3
                })
            else
                WindUI:Notify({
                    ...
                    Content = "ไม่สามารถโหลดสคริปต์ DIAMOND F",
                    Icon = "x-circle",
                    Duration = 5
                })
            end
            ...
            WindUI:Notify({
                Title = "Diamond Farm ปิดใช้งาน",
                Content = "ปิดการใช้งาน - สคริปต์อาจยังทำงานอยู่",
                Icon = "info",
                Duration = 2
            })
        end
...
local PlayerDropdown = CollectorTab:Dropdown({
    Title = "รวบรวมไอเท็มไปยังผู้เล่น",
    Desc = "",
    ...
    Callback = function(Value)
        ...
        WindUI:Notify({
            Title = "เปลี่ยนผู้เล่นเป้าหมายแล้ว",
            Content = "ไอเท็มจะถูกรวบรวมไปยัง" .. Value,
            Duration = 2
        })
    end
})
...
CollectorTab:Toggle({
    Title = "ปล้นไอเท็มใหม่โดยอัตโนมัติ",
...
PivotTab:Button({
    Title = "เทเลพอร์ตไปยังตำแหน่งที่เลือก",
    Desc = "",
    Icon = "map-pin",
    Callback = function()
...
CombatTab:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({
            Title = "แก้ไขข้อความแจ้งแล้ว",
            Content = "ตั้งค่า" .. modified .. " prompts to instant",
            Duration = 2
        })
    end
})
...
CookingTab:Toggle({
    Title = "การรับประทานอาหารอัตโนมัติ",
    Desc = "",
    Value = false,
    Callback = function(Value)
...
-- =====================
VisualTab:Button({
    Title = "การเพิ่มประสิทธิภาพการมองเห็น",
    Desc = "",
    Icon = "info",
    Callback = function() end
})
...
VisualTab:Button({
    Title = "คัดลอกสคริปต์เทเลพอร์ตเกม",
    Desc = "",
    Icon = "copy",
    Callback = function()
...
LogWallTab:Button({
    Title = "⚠️ โหมดเบต้า - คาดหวังข้อบกพร่อง",
    Desc = "",
    Icon = "alert-triangle",
    Color = Color3.fromHex("#ff4830"),
    Callback = function() end
...
resourceButton = LogWallTab:Button({
    Title = "ทรัพยากรที่มีอยู่",
    Desc = "",
    Icon = "package",
    Callback = function()
        updateLogWallResourceDisplay()
        WindUI:Notify({
            Title = "อัปเดตแหล่งข้อมูลแล้ว",
            Content = "",
            Icon = "refresh-cw",
            Duration = 2
        })
...
LogWallTab:Button({
    Title = "ล้างการแสดงตัวอย่าง",
    Desc = "",
    Icon = "trash-2",
    Callback = function()
        LogWallBuilder:ClearPreview()
        WindUI:Notify({
            Title = "ล้างการแสดงตัวอย่างแล้ว",
            Content = "",
            Icon = "trash-2",
            Duration = 2
        })
...
plantResourceButton = PlantHouseTab:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({
            Title = "อัปเดตแหล่งข้อมูลแล้ว",
            Content = "จำนวนรายการได้รับการรีเฟรชแล้ว",
            Icon = "refresh-cw",
            Duration = 2
        })
    end
    ...
    Callback = function()
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            WindUI:Notify({
                ...
                Content = "ตั้งค่าเป็นตำแหน่งผู้เล่น",
                Icon = "map-pin",
                Duration = 2
            })
        end
...
PlantHouseTab:Button({
    ...
    Callback = function()
        if PlantHouseBuilder.config.isBuilding then
            WindUI:Notify({
                Title = "กำลังสร้างอยู่แล้ว",
                Content = "โปรดรอให้บิลด์ปัจจุบันเสร็จสิ้น",
                Icon = "alert-circle",
                Duration = 3
            })
            return
        ...
        if #allPoints > 0 then
            ...
            if available < #allPoints then
                WindUI:Notify({
                    Title = "ไอเท็มไม่พอ!",
                    Content = "ความต้องการ" .. (#allPoints - available) .. " more items",
                    Icon = "x-circle",
                    Duration = 5
                })
                return
            ...
            WindUI:Notify({
                Title = "เริ่มสร้างแล้ว!",
                Content = "อาคาร" .. #allPoints .. " blocks",
                Icon = "hammer",
                Duration = 3
            })
        else
            WindUI:Notify({
                Title = "ไม่ได้เลือกอะไร",
                Content = "โปรดเลือกรูปทรงโรงงานหรือประเภทบ้าน",
                Icon = "alert-triangle",
                Duration = 3
            })
        end
...
PlantHouseTab:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({
            Title = "อาคารหยุดทำงาน",
            Content = "ยกเลิกบิลด์ที่รอดำเนินการทั้งหมดแล้ว",
            Icon = "square",
            Duration = 2
        })
    end
...
ConfigTab:Button({
    ...
    Callback = function()
        if configNameInput == "" then
            WindUI:Notify({
                Title = "ข้อผิดพลาด",
                Content = "ป้อนชื่อการกำหนดค่าก่อน",
                Icon = "alert-circle",
                Duration = 3
            })
            return
...
ConfigTab:Button({
    Title = "บันทึกการกำหนดค่าปัจจุบัน",
...
ConfigTab:Button({
    Title = "โหลดและดำเนินการ (กำแพงบันทึก)",
    ...
    Callback = function()
        ...
        if not success then
            ...
            WindUI:Notify({
                Title = "กำลังประมวลผลผนังปูมบันทึก",
                Content = selectedConfig,
                Icon = "zap",
                Duration = 2
            })
...
autoLoadToggle = ConfigTab:Toggle({
    Title = "โหลดอัตโนมัติเมื่อเริ่มต้น",
...
autoLoadConfigDropdown = ConfigTab:Dropdown({
    Title = "กำหนดค่าการโหลดอัตโนมัติ",
    ...
    Callback = function(v)
        ...
        saveAutoLoadSettings()
        WindUI:Notify({
            Title = "ตั้งค่าการกำหนดค่าการโหลดอัตโนมัติ",
            Content = v,
            Icon = "check-circle",
            Duration = 2
        })
...
WindUI:Notify({
    Title = "โหลดศูนย์เครื่องมือระดับสูงสุดแล้ว!",
    Content = "ฟีเจอร์ทั้งหมดพร้อมแล้ว",
    Icon = "check-circle",
    Duration = 3
})
