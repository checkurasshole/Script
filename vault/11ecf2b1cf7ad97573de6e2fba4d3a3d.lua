-- Spanish Translation
-- Script ID: 11ecf2b1cf7ad97573de6e2fba4d3a3d
-- Updated: 2025-12-18T07:42:07.250Z
...
local Tabs = {
    Respawn = Window:AddTab("Reaparición Automática", "refresh-cw"),
    AimAssist = Window:AddTab("Asistente de Puntería", "star"),
    Visuals = Window:AddTab("Visuales", "eye"),
    ["UI Settings"] = Window:AddTab("Configuración de UI", "settings"),
}

local AutoPlayBox = Tabs.Respawn:AddLeftGroupbox("Bucle de Juego Automático")
local AimbotV1Box = Tabs.AimAssist:AddLeftGroupbox("Versión 1 - PC Manual")
local AimbotV2Box = Tabs.AimAssist:AddRightGroupbox("Versión 2 - Automático")
local TriggerBox = Tabs.AimAssist:AddLeftGroupbox("Disparo Automático")
local ESPBox = Tabs.Visuals:AddLeftGroupbox("ESP de Jugador")
local TracerBox = Tabs.Visuals:AddRightGroupbox("Trazadores")

Module.Initialize(Library, Options, Toggles)

AutoPlayBox:AddToggle("AutoPlay", {
    Text = "Bucle de Juego Automático",
    Default = false,
    Tooltip = "Reaparición automática al morir",
})
...
AimbotV1Box:AddToggle("AimbotV1", {
    Text = "Activar",
    Default = false,
    Tooltip = "Mantén clic derecho para bloquear",
})
...
AimbotV2Box:AddToggle("AimbotV2", {
    Text = "Activar",
    Default = false,
    Tooltip = "Bloqueo automático sin botón",
})
...
TriggerBox:AddToggle("TriggerBot", {
    Text = "Activar",
    Default = false,
    Tooltip = "Disparo automático al bloquear",
})
                ...
                if player ~= LocalPlayer and Module.isAlive(player) then
                    ...
                    if hrp then
                        ...
                        if onScreen then
                            ...
                            local fromPos
                            if Options.TracerPosition.Value == "Arriba" then
                                fromPos = Vector2.new(Camera.ViewportSize.X / 2, 0)
                            elseif Options.TracerPosition.Value == "Centro" then
...
MenuGroup:AddLabel("Atajo de Menú"):AddKeyPicker("MenuKeybind", {
    Default = "RightShift",
    NoUI = true,
    Text = "Alternar Menú"
})
