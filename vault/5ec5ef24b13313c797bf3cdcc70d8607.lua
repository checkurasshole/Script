-- Spanish Translation
-- Script ID: 5ec5ef24b13313c797bf3cdcc70d8607
-- Updated: 2026-09-14T17:51:41.961Z
...
local Tab = Window:AddTab("Robar", "shopping-bag")
local Box = Tab:AddLeftGroupbox("Robar huevos")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Áreas" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Rarezas" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mutaciones" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Prioridad de objetivo" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Método de robo", Tooltip = "Tras el golpe del pollo: Instant TP se pega al huevo (lo más rápido); Second Method corre hacia él por el carril como el auto-steal antiguo (sin teletransporte)" })
Box:AddToggle("se_selected",   { Text = "Robo automático", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Pasos automáticos (elige qué hace el farm)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "No vender (rarezas)" })
Box:AddToggle("se_return",     { Text = "Volver a la base automáticamente", Default = true })
Box:AddButton({ Text = "Salir de la cinta", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Info")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Robo: Activo")
InfoBox:AddButton({ Text = "Reescanear áreas", Func = function()
    local areas = {}
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Buggeado") .. " -> reinicio por respawn")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: API bloqueada por el ejecutor (0 servidores)" or "Hop: ningún servidor disponible, reintenta")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d servidores, el más vacío %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
    local function tryNext()
        idx = idx + 1
        local s = order[idx]
        if not s then if conn then conn:Disconnect() end; hopping = false; notify("Hop: todos fallaron, reintenta"); return end
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("No hay bate para equipar") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Bate equipado por el slot de la hotbar " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("Las teclas de la hotbar no lo equiparon - mantén la ventana de Roblox enfocada y reintenta") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Movimiento")
MoveBox:AddToggle("se_tpwalk", { Text = "Velocidad", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Velocidad de caminata", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Vuelo", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Velocidad de vuelo", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Anti empuje", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Anti golpe (recoger si se cae)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "ESP de huevos (todos los huevos)", Default = false })
MoveBox:AddButton({ Text = "suicidio", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Reinicio (respawn) al fallar", Default = true, Tooltip = "Cuando un ciclo de robo se buggea y te deja tirado en el campo, reaparece en la base (vía suicidio) en vez de volver lentamente -- reinicio mucho más rápido" })

local ServerBox = Tab:AddRightGroupbox("Servidor")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Preferencia de salto" })
ServerBox:AddButton({ Text = "Cambiar de servidor", Func = function()
    local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
    task.spawn(function() serverHop(pref) end)
end })
ServerBox:AddButton({ Text = "Reingresar", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "Auto Hop cuando esté lleno", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Saltar con jugadores >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "Cuando Auto Hop está activo y el servidor alcanza esta cantidad de jugadores, salta a un servidor nuevo (usa la Preferencia de salto)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Servidor lleno (" .. #Players:GetPlayers() .. ") -> saltando")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            local disp
            if FPS_PAUSED then disp = "PAUSADO - FPS bajo (" .. fps .. ")"
            elseif not stealAllowed() then disp = "Pausado (reinicio de huevos)"
            elseif os.clock() - statusHintAt < 2.5 then disp = statusHint
            elseif on("se_selected") or on("se_bloom") then disp = "Activo"
            else disp = "Inactivo" end
            stateLabel:SetText("Estado: " .. tostring(disp))
        end)
        task.wait(0.5)
    end
end)
...
local function mountTread()
    local part = treadTop(); if not part then setStatus("Cinta no encontrada"); return end
    setStatus("Yendo a la cinta")
    ...
    onTread = true
    setStatus("En la cinta (sin huevos)")
end
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("Pausado (reinicio de huevos)"); return end
            local list = {}
            for _, m in ipairs(scanEggs()) do if passesFilters(m.Name, true) then list[#list + 1] = m end end
            if #list == 0 then
                if step("Treadmill") then mountTread() else forceDoff(); setStatus("Esperando huevos") end
                return
            end
            ...
            if not fr then setStatus("No hay huevo del bosque para preparar"); unlockMove(); return end
            setStatus("Preparar: huevo del bosque")
            ...
            setStatus("Esperando el golpe del pollo...")
            ...
            if not hit then setStatus("No se detectó golpe en 25s"); resetToBase("sin golpe de pollo"); unlockMove(); return end
            ...
            if not haveIt() then setStatus("Fallo al agarrar el objetivo"); resetToBase("fallo al agarrar"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        if not on("se_antihit") then setStatus("Huevo caído (anti golpe apagado)"); break end
                        setStatus("Huevo caído -> volviendo por él")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("fallo al volver") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
        if not ok then unlockMove(); setStatus("Recuperando...") end
        task.wait(0.2)
    end
end)

local EventsTab = Window:AddTab("Eventos", "sparkles")
local BloomBox = EventsTab:AddLeftGroupbox("Gran Floración")
BloomBox:AddToggle("se_bloom", { Text = "Auto farmear Bloom", Default = false })
local FuseBox = EventsTab:AddLeftGroupbox("Máquina de fusión")
FuseBox:AddToggle("se_autofuse", { Text = "Auto fusionar mascotas", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Incubadora Sakura")
local crysLabel = IncBox:AddLabel("Cristales: 0")
IncBox:AddToggle("se_incubate", { Text = "Auto incubar", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Auto depositar cristales", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Auto mutar cuando esté lleno", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Evento de Jefe")
BossBox:AddToggle("se_boss", { Text = "Auto jefe (entrar + golpear cristales)", Default = false })
local bossLabel = BossBox:AddLabel("Jefe: inactivo")
local bossCountLabel = BossBox:AddLabel("Próximo jefe: --:--")
...
task.spawn(function()
    while RUNNING do
        if on("se_boss") and BOSS then
            if not inArena() then
                pcall(function() bossLabel:SetText("Jefe: entrando") end)
                ...
                if #crystals > 0 then
                    ...
                    pcall(function() bossLabel:SetText("Jefe: cristales (" .. #crystals .. " restantes)") end)
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Jefe: atacando brazos (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Jefe: atacando al jefe (sin brazo)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Jefe: esperando (cambio de fase)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        local s = save()
        pcall(function() crysLabel:SetText("Cristales: " .. tostring(s and s.SakuraCrystals or 0)) end)
        task.wait(1)
    end
end)
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Jefe: ACTIVO (en la arena)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Próximo jefe en ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Luz vs Oscuridad")
    LvdBox:AddToggle("se_lvd", { Text = "Auto Luz vs Oscuridad", Default = false })
    LvdBox:AddDropdown("se_lvdteam", { Values = { "Random", "Light", "Darkness" }, Default = "Random", Text = "Equipo" })
    local lvdLabel = LvdBox:AddLabel("LvD: inactivo")
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Luz vs Oscuridad EMPEZÓ -- " .. fmtT(dur) .. " restante", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: uniéndose al equipo") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " restante") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " anillos" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: esperando (iniciado por el staff, sin temporizador)" or "LvD: inactivo") end)
                task.wait(1)
            end
        end
    end)
    ...
    while RUNNING do
        local ok = pcall(function()
            ...
            for _, tree in ipairs(CollectionService:GetTagged("SakuraBloomTree")) do
                ...
                if okp and pos then
                    lockMove()
                    setStatus("Bloom: golpeando el árbol")
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Bloom: recogiendo cristal")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("No hay trío para fusionar"); return end
                setStatus("Fusionando mascotas")
...
local ProtBox = SettingsTab:AddLeftGroupbox("Protección")
ProtBox:AddToggle("se_antiafk", { Text = "Anti AFK", Default = true })
ProtBox:AddToggle("se_antilag", { Text = "Reducir lag", Default = false, Tooltip = "Elimina partículas/sombras/efectos, baja calidad y OCULTA las mascotas de otros jugadores (del lado del cliente) para detener el pico de lag al teletransportar", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Áreas")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Noclip de guardias", Default = true, Tooltip = "Hace que los animales guardia sean atravesables (del lado del cliente) para que no te bloqueen ni te empujen al robar", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "Bloquear hora" })
...
local MenuBox = SettingsTab:AddRightGroupbox("Menú")
MenuBox:AddButton({ Text = "Descargar", Func = function()
...
MenuBox:AddLabel("Alternar menú"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "Alternar menú" })
...
local FG = SuggestTab:AddLeftGroupbox("Sugerencias / Ayuda", "message-square")
    ...
    Func = function()
        ...
        task.spawn(function()
            ...
            if sok then
                ...
                notify("Tu comentario fue enviado. ¡Gracias!")
            else
...
local HelpGroup = SuggestTab:AddRightGroupbox("Ayuda / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Unirse a Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Enlace de Discord copiado. Pégalo en tu navegador.")
    end,
})
HelpGroup:AddDivider()
HelpGroup:AddLabel("¿Tienes una sugerencia o encontraste un bug?\nElige un tipo, escribe tu mensaje y presiona Enviar.", true)

notify("COMBO_WICK cargado")
