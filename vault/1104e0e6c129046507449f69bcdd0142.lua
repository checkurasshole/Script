...
-- Script ID: 1104e0e6c129046507449f69bcdd0142
-- Migrated: 2025-09-11T13:21:01.071Z
-- Auto-migrated from encrypted storage to GitHub

local Fluent = loadstring(game:HttpGet("__URL_ddf4cc95eaa3ac4d__"))()

local Window = Fluent:CreateWindow({
    Title = "Автоматическая ферма & продажа",
    SubTitle = "",
    TabWidth = 80,
...
local Tab = Window:AddTab({ Title = "Главное", Icon = "" })
local ShopTab = Window:AddTab({ Title = "Магазин", Icon = "" })
...
Tab:AddToggle("AutoFarm", {Title = "Автоматическая ферма", Default = false}):OnChanged(function(value)
...
Tab:AddToggle("AutoSell", {Title = "Автопродажа", Default = false}):OnChanged(function(value)
...
ShopTab:AddParagraph({
    Title = "Обычные предметы",
    Content = "Добавьте свои собственные вещи к автопокупке"
})
...
ShopTab:AddButton({
    Title = "Добавить название продукта в магазин",
    ...
    Callback = function()
        ...
        if customItemName ~= "" and customItemName ~= nil and not customToggles[customItemName] then
            ...
            -- Create the toggle
            ShopTab:AddToggle(toggleId, {
                Title = "Автопокупка" .. customItemName,
                Default = false
            }):OnChanged(function(value)
