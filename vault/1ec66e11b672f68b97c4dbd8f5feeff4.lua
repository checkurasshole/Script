-- Script_1754747679382
-- Script ID: 1ec66e11b672f68b97c4dbd8f5feeff4
-- Migrated: 2025-09-11T12:58:17.447Z
-- Auto-migrated from encrypted storage to GitHub

for _, url in ipairs{
    "__URL_eee79dddeed62490__",
    "__URL_f386bc7c8d97f98f__"
} do
    local s = loadstring(game:HttpGet(url))
    if s then s() end
end

local Rayfield = loadstring(game:HttpGet('__URL_14fe90e6fe2a7d86__'))()
...
local MainSection = MainTab:CreateSection("Controles ESP")
...
local TeammatesToggle = MainTab:CreateToggle({
   Name = "Mostrar compañeros de equipo",
   CurrentValue = true,
...
local RefreshHitboxButton = MainTab:CreateButton({
    Name = "Actualizar Hitbox",
    Callback = function()
        refreshHitbox()
        Rayfield:Notify({
            Title = "Hitbox Actualizado",
            Content = "¡Todos los hitboxes han sido reaplicados!",
            Duration = 2,
        })
    end,
})

local VisualsSection = VisualsTab:CreateSection("Elementos visuales")
...
local RenderDistanceSlider = VisualsTab:CreateSlider({
   Name = "Distancia de renderización",
   Range = {100, 2000},
   Increment = 50,
   CurrentValue = 1000,
   Flag = "RenderDistance",
...
local TeleportSection = TeleportTab:CreateSection("Controles de teletransporte")
...
local TeleportDistanceSlider = TeleportTab:CreateSlider({
   Name = "Distancia de teletransporte",
   Range = {1, 50},
   Increment = 1,
   CurrentValue = 7,
...
local AimbotSection = AimbotTab:CreateSection("Configuración automática de...")
...
local UnlockTargetButton = AimbotTab:CreateButton({
   Name = "Obligar a desbloquear el ob...",
   Callback = function()
       unlockTarget()
   end,
})

local TargetStatusSection = AimbotTab:CreateSection("Estado de destino")
...
Rayfield:Notify({
   Title = "ESP, Aimbot y Hitbox cargados",
   Content = "¡Los sistemas mejorados de ESP, aimbot y hitbox con funcionalidad de teletran...",
   Duration = 3,
   Image = "eye"
})