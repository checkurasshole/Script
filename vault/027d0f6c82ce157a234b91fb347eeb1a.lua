...
-- Script ID: 027d0f6c82ce157a234b91fb347eeb1a
-- Migrated: 2025-09-11T13:21:01.929Z
-- Auto-migrated from encrypted storage to GitHub

local Fluent = loadstring(game:HttpGet("__URL_df6674e0d68cfe65__"))()

local Window = Fluent:CreateWindow({
    Title = "Auto Farm & Sell",
    SubTitle = "",
    TabWidth = 80,
...
local Tab = Window:AddTab({ Title = "Principale", Icon = "" })
local ShopTab = Window:AddTab({ Title = "Negozio", Icon = "" })
...
Tab:AddToggle("AutoFarm", {Title = "Fattoria automatica", Default = false}):OnChanged(function(value)
...
Tab:AddToggle("AutoSell", {Title = "Vendita automatica", Default = false}):OnChanged(function(value)
...
for _, itemName in ipairs(shopItems) do
    ...
    ShopTab:AddToggle(toggleId, {
        Title = "Acquisto automatico" .. itemName, 
        Default = false
...
ShopTab:AddParagraph({
    Title = "Articoli lavorati in modo personalizzato",
    Content = "Aggiungi i tuoi articoli all'acquisto automatico"
})
...
ShopTab:AddButton({
    Title = "Aggiungi il nome dell'articolo del negozio",
    ...
    Callback = function()
        ...
        if customItemName ~= "" and customItemName ~= nil and not customToggles[customItemName] then
            ...
            -- Create the toggle
            ShopTab:AddToggle(toggleId, {
                Title = "Acquisto automatico" .. customItemName,
                Default = false
            }):OnChanged(function(value)
