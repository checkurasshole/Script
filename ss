...
function Module.isPlayerNearby(pos)
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= Module.player and p.Character then
			...
			if r then
				local collectionRadius = p.Character:FindFirstChild("FoodCollectionRadius")
				local safeDistance = Module.SAFETY_BUFFER
				
				if collectionRadius and collectionRadius:IsA("NumberValue") then
					safeDistance = collectionRadius.Value + Module.SAFETY_BUFFER
				elseif collectionRadius and typeof(collectionRadius) == "number" then
					safeDistance = collectionRadius + Module.SAFETY_BUFFER
				else
					local radius = p.Character:GetAttribute("FoodCollectionRadius")
					if radius then
						safeDistance = radius + Module.SAFETY_BUFFER
					else
						safeDistance = 80
					end
				end
				
				if (pos - r.Position).Magnitude < safeDistance then
					return true
				end
			end
		end
