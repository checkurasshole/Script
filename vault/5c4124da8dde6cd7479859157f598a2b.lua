-- Script_1754579624585
-- Script ID: 5c4124da8dde6cd7479859157f598a2b
-- Migrated: 2025-09-11T12:58:14.829Z
...
local Rayfield = loadstring(game:HttpGet('__URL_e398fbe8f47a0d38__'))()
...
local function applyHighlight(model)
    ...
    highlight.Name = "НПК_Высокий свет"
...
local Window = Rayfield:CreateWindow({
    Name = "COMBO_WICK",
    Icon = 12345678901,
    LoadingTitle = "Наслаждайтесь!",
    LoadingSubtitle = "По COMBO_WICK",
    Theme = "Ocean",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "КомбоХроническая",
        FileName = "NCCCConfig"
    }
})

local Tab = Window:CreateTab("Главное", nil)
local AmberTab = Window:CreateTab("Коллекция автожелтых", nil)
local UtilityTab = Window:CreateTab("Коммунальное обслуживание", nil)

-- Utility Tab (Anti-Void)
UtilityTab:CreateSection("Противоядие")
...
UtilityTab:CreateButton({
    Name = "Подделка (Телепорт под картой)",
    Callback = function()
        fakeOut()
    end
})
...
-- Main Tab
Tab:CreateSection("Скорость движения")
Tab:CreateToggle({
    Name = "Включить ходовую скорость",
    CurrentValue = false,
...
Tab:CreateSlider({
    Name = "Значение скорости движения",
    Range = {1, 500},
    Increment = 1,
    Suffix = "speed",
    CurrentValue = 16,
...
Tab:CreateSection("Автомобиль для животных")
Tab:CreateToggle({
    Name = "Использовать имя ESP",
    CurrentValue = false,
    Flag = "ESPNameToggle",
    Callback = function(value)
        toggleESP(value)
...
Tab:CreateButton({
    Name = "Следующий целевой показател...",
    Callback = function()
...
Tab:CreateSection("Проигрыватель AutoFarm")
Tab:CreateToggle({
    Name = "Телепорт для игроков",
    CurrentValue = false,
    Flag = "TeleportToggle",
    Callback = function(value)
        teleporting = value
...
AmberTab:CreateSection("Автомобиль автофургона")
AmberTab:CreateToggle({
    Name = "Использовать коллекцию авто...",
    CurrentValue = false,
