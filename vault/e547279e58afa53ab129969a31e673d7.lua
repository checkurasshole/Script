-- French Translation
-- Script ID: e547279e58afa53ab129969a31e673d7
-- Updated: 2026-01-12T02:45:55.047Z
...
if not versionValid then
    Library:Notify({
        Title = 'Erreur de version',
        Description = versionMessage,
        Time = 10,
    })
    return {
        success = false,
        error = "échec_de_vérification_de_version",
...
if VaultModule.compareVersions(VaultModule.CONFIG.VERSION, '1.3.2') <= 0 then
    Library:Notify({
        ...
        Description = 'Cette version n\'est plus prise en charge. Veuillez mettre à jour vers la version ' .. VaultModule.CONFIG.MIN_VERSION .. ' ou supérieure.',
        Time = 10,
    })
    return {
        success = false,
        error = "version_obsolète",
        message = "Version n'est plus prise en charge"
    }
end
...
if not whitelisted or errorType == "access_denied" or errorType == "tamper_detected" or errorType == "key_expired" then
    local Window = Library:CreateWindow({
        Title = "COMBO_WICK",
        Footer = "Système de validation v" .. VaultModule.CONFIG.VERSION,
        Icon = 18928661286,
    ...
    local Tabs = {
        ...
        Info = Window:AddTab("Informations et liens", "info")
    }
        
    local validationComplete = false
    local validationSuccess = false
    
    local MainGroup = Tabs.Validation:AddLeftGroupbox("Entrez votre clé API")
    ...
    MainGroup:AddButton({
        ...
        Func = function()
            ...
            if key == "" then
                Library:Notify({
                    Title = "Erreur : clé vide",
                    Description = "Veuillez entrer une clé de validation",
                    Time = 3
                })
                return
            end
    ...
    MainGroup:AddButton({
        Text = "Obtenir le lien de clé",
        Func = function()
            setclipboard("https://chronicle-secure-admin-dashboard-kappa.vercel.app/verify/provider-select")
            Library:Notify({
                Title = "Copié",
                Description = "Lien de clé copié dans le presse-papiers",
                Time = 2
            })
        end
    })
    
    local LinksGroup = Tabs.Info:AddLeftGroupbox("Liens utiles")
    ...
    LinksGroup:AddButton({
        ...
        Func = function()
            ...
            Library:Notify({
                Title = "Copié",
                Description = "Lien YouTube copié dans le presse-papiers",
                Time = 2
            })
        end
    })
    ...
    LinksGroup:AddButton({
        ...
        Func = function()
            ...
            Library:Notify({
                Title = "Copié",
                Description = "Lien Discord copié dans le presse-papiers",
                Time = 2
            })
        end
    })
    
    local HWIDGroup = Tabs.Info:AddRightGroupbox("Informations sur l'ID matériel")
    
    HWIDGroup:AddLabel({
        Text = "Votre ID matériel :",
        DoesWrap = false,
    })
    ...
    HWIDGroup:AddButton({
        Text = "Copier l'ID matériel",
        Func = function()
            setclipboard(clientId)
            Library:Notify({
                Title = "Copié",
                Description = "ID matériel copié dans le presse-papiers",
                Time = 2
            })
        end
    })
    ...
    if validationSuccess then
        ...
        if whitelisted then
            ...
            Library:Notify({
                Title = "Chargement",
                Description = "Chargement du script...",
                Time = 2
            })
            ...
            return {
                success = false,
                error = "non_autorisé_après_validation"
            }
        end
    else
        return {
            success = false,
            error = "validation_annulée"
        }
    end
end
