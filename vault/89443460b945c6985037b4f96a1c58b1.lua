...
-- Script ID: 89443460b945c6985037b4f96a1c58b1
-- Created: 2025-10-19T14:52:14.826Z
-- Source: External API
...
local Tabs = {
    Main = Window:AddTab("Радиолокатор", "eye"),
    AutoWalk = Window:AddTab("Охота на цель", "navigation"),
    Collect = Window:AddTab("Претензия в отношении денежных средств", "dollar-sign"),
    PrivServer = Window:AddTab("Серверный режим", "server"),
    ["UI Settings"] = Window:AddTab("Параметры", "settings"),
}
...
local ESPGroupBox = Tabs.Main:AddLeftGroupbox("Параметры отображения", "eye")

ESPGroupBox:AddToggle("ЭСПТоггл", {
    Text = "Показать пит-маркеры",
    Default = false,
    Callback = function(v)
        espEnabled = v
        if not v then
...
ESPGroupBox:AddToggle("Toggle TrickerToggle", {
    Text = "Показать направления",
    Default = false,
    Callback = function(v)
        tracerEnabled = v
        if not v then
...
local AnimalGroupBox = Tabs.Main:AddRightGroupbox("Фильтр Pet", "filter")

AnimalGroupBox:AddDropdown("CommonDropdown", {
    Values = ComboWick.commonAnimals,
    Multi = true,
    Default = 1,
    Text = "Стандартные питомцы",
...
AnimalGroupBox:AddDropdown("AdminDropdown", {
    ...
    Multi = true,
    Default = 1,
    Text = "Административные органы/ОИ Пешеходы",
    Callback = function(Value)
...
local AutoWalkGroupBox = Tabs.AutoWalk:AddLeftGroupbox("Параметры охоты", "navigation")

AutoWalkGroupBox:AddToggle("АвтоWalkToggle", {
    Text = "Начинайте охоту",
    Default = false,
    Callback = function(v)
        autoWalkEnabled = v
        if v then
...
local CollectGroupBox = Tabs.Collect:AddLeftGroupbox("Параметры претензии", "dollar-sign")

CollectGroupBox:AddToggle("Сбор денег", {
    Text = "Автомобильная претензия",
    Default = false,
    Callback = function(v)
...
local StatusGroupBox = Tabs.Collect:AddRightGroupbox("Статус", "activity")
...
local PrivServerGroupBox = Tabs.PrivServer:AddLeftGroupbox("Частный сервер", "server")
...
local MenuGroup = Tabs["UI Settings"]:AddLeftGroupbox("Меню", "settings")
