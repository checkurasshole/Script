-- English (Original)
-- Script ID: 94cb8d3afa8ae7f64bca0940f6f40e7f
-- Updated: 2026-09-14T17:51:32.030Z
...
local Tab = Window:AddTab("Steal", "shopping-bag")
local Box = Tab:AddLeftGroupbox("Steal Eggs")
Box:AddDropdown("se_areas",     { Values = {}, Default = {}, Multi = true, AllowNull = true, Text = "Areas" })
Box:AddDropdown("se_rarities",  { Values = RARITY_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Rarities" })
Box:AddDropdown("se_mutations", { Values = MUTATION_LIST, Default = {}, Multi = true, AllowNull = true, Text = "Mutations" })
Box:AddDropdown("se_priority",  { Values = { "Nearest", "Highest Value", "Highest Weight" }, Default = "Nearest", Text = "Target Priority" })
Box:AddDropdown("se_stealmethod", { Values = { "Instant TP", "Second Method" }, Default = "Instant TP", Text = "Steal Method", Tooltip = "After the chicken hit: Instant TP snaps onto the egg (fastest); Second Method runs to it via the lane like the old auto-steal (no teleport)" })
Box:AddToggle("se_selected",   { Text = "Auto Steal", Default = false, Callback = function(v) if v then inv("LeaveTread") end end })
Box:AddDropdown("se_steps", { Values = { "Place", "Hatch", "Sell", "Treadmill" }, Default = {}, Multi = true, AllowNull = true, Text = "Auto Steps (pick what the farm does)" })
Box:AddDropdown("se_keepsell", { Values = RARITY_LIST, Multi = true, AllowNull = true, Default = { Legendary = true, Mythic = true, Mythical = true, Rainbow = true, Cosmic = true, Exclusive = true, Secret = true, Exotic = true, Eternal = true, Limited = true, Superior = true, Divine = true, Titan = true, BrainrotGod = true, LightDark = true }, Text = "Don't Sell (rarities)" })
Box:AddToggle("se_return",     { Text = "Auto Return to Base", Default = true })
Box:AddButton({ Text = "Leave Treadmill", Func = function() inv("LeaveTread") end })

local InfoBox = Tab:AddRightGroupbox("Info")
local statLabel = InfoBox:AddLabel("FPS: -")
local stateLabel = InfoBox:AddLabel("Steal: Active")
InfoBox:AddButton({ Text = "Rescan Areas", Func = function()
    local areas = {}
...
local function resetToBase(reason)
    ...
    setStatus((reason or "Bugged") .. " -> respawn reset")
    suicide()
    local t0 = os.clock()
...
local function serverHop(pref)
    ...
    if #list == 0 then
        notify(scanned == 0 and "Hop: API blocked by executor (0 servers)" or "Hop: no joinable server, retry")
        hopping = false
        return
    end
    ...
    notify(("Hop: %d servers, emptiest %d"):format(#list, list[1] and list[1].playing or 0))
    local idx = 0
    local conn
...
local function equipBatInput(silent)
    ...
    if not owns() then
        ...
        if heldBat() then return true end
        if not owns() then if not silent then notify("No bat available to equip") end return false end
    end
    for i = 1, 10 do
        pcall(function()
        ...
        task.wait(0.08)
        if heldBat() then if not silent then notify("Equipped bat via hotbar slot " .. (i == 10 and 0 or i)) end return true end
    end
    if not silent then notify("Hotbar keys didn't equip it - keep the Roblox window focused and try again") end
    return false
end

local MoveBox = Tab:AddRightGroupbox("Movement")
MoveBox:AddToggle("se_tpwalk", { Text = "Walkspeed", Default = true })
MoveBox:AddSlider("se_walk",   { Text = "Walk Speed", Default = 974, Min = 0, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_fly", { Text = "Flight", Default = false })
MoveBox:AddSlider("se_flyspeed", { Text = "Flight Speed", Default = 60, Min = 20, Max = 1000, Rounding = 0 })
MoveBox:AddToggle("se_antikb", { Text = "Anti Knockback", Default = true })
MoveBox:AddToggle("se_antihit",    { Text = "Anti Hit (re-grab if dropped)", Default = true })
MoveBox:AddToggle("se_esp", { Text = "Egg ESP (all eggs)", Default = false })
MoveBox:AddButton({ Text = "suicide", Func = function() task.spawn(suicide) end })
MoveBox:AddToggle("se_suicidereset", { Text = "Respawn Reset on Fail", Default = true, Tooltip = "When a steal cycle bugs out and strands you in the field, respawn at base (via suicide) instead of slowly tweening back -- much faster restart" })

local ServerBox = Tab:AddRightGroupbox("Server")
ServerBox:AddDropdown("se_hoppref", { Values = { "Lowest Players", "Highest Players" }, Default = "Lowest Players", Text = "Hop Preference" })
...
ServerBox:AddButton({ Text = "Rejoin", Func = function()
    pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, lp) end)
end })
ServerBox:AddToggle("se_autohop", { Text = "Auto Hop when full", Default = false })
ServerBox:AddSlider("se_hopat", { Text = "Hop at Players >=", Default = 8, Min = 2, Max = 50, Rounding = 0, Tooltip = "When Auto Hop is on and the server reaches this many players, hop to a new server (uses Hop Preference)" })
task.spawn(function()
    while RUNNING do
        if on("se_autohop") then
            ...
            if #Players:GetPlayers() >= n then
                local pref = (Opt.se_hoppref and Opt.se_hoppref.Value) or "Lowest Players"
                setStatus("Server full (" .. #Players:GetPlayers() .. ") -> hopping")
                serverHop(pref)
                task.wait(15)
            end
        end
...
task.spawn(function()
    while RUNNING do
        ...
        pcall(function()
            ...
            elseif not stealAllowed() then disp = "Paused (egg reset)"
...
local function mountTread()
    ...
    setStatus("Heading to treadmill")
...
task.spawn(function()
    while RUNNING do
        local ok = pcall(function()
            ...
            if not stealAllowed() then setStatus("Paused (egg reset)"); return end
            local list = {}
            ...
            if not fr then setStatus("No forest egg to prime"); unlockMove(); return end
            setStatus("Prime: forest egg")
            ...
            if not hit then setStatus("No hit detected in 25s"); resetToBase("no chicken hit"); unlockMove(); return end
            ...
            if (Opt.se_stealmethod and Opt.se_stealmethod.Value) == "Second Method" then
                clearRagdoll(); setStatus("Walking to egg (lane)")
            ...
            if not haveIt() then setStatus("Target grab failed"); resetToBase("grab failed"); unlockMove(); return end
            stolen = stolen + 1
            if on("se_return") then
                ...
                while keepGoing() and not delivered and os.clock() - guard < 45 do
                    ...
                    if not fpCarrying then
                        ...
                        setStatus("Egg dropped -> going back for it")
                        local gg = os.clock()
                ...
                if not delivered and not fpCarrying then resetToBase("return failed") end   -- egg already lost -> safe to respawn
            end
            unlockMove()
        end)
...
FuseBox:AddToggle("se_autofuse", { Text = "Auto Fuse Pets", Default = false })
local IncBox = EventsTab:AddRightGroupbox("Sakura Incubator")
local crysLabel = IncBox:AddLabel("Crystals: 0")
IncBox:AddToggle("se_incubate", { Text = "Auto Incubate", Default = false })
IncBox:AddToggle("se_autodeposit", { Text = "Auto Deposit Crystals", Default = true })
IncBox:AddToggle("se_automutate", { Text = "Auto Mutate When Full", Default = true })

local BossBox = EventsTab:AddRightGroupbox("Boss Event")
BossBox:AddToggle("se_boss", { Text = "Auto Boss (enter + hit crystals)", Default = false })
local bossLabel = BossBox:AddLabel("Boss: idle")
local bossCountLabel = BossBox:AddLabel("Next boss: --:--")
            ...
            if not inArena() then
                ...
                if #crystals > 0 then
                    ...
                    if #arms > 0 then
                        ...
                        pcall(function() bossLabel:SetText("Boss: attacking arms (" .. #arms .. ")") end)
                        orbitAndSwing(arms[1], keep)
                    else
                        local body = bossBodyPart()
                        if body then
                            pcall(function() bossLabel:SetText("Boss: attacking boss (no arm up)") end)
                            orbitAndSwing(body, keep)
                        else
                            pcall(function() bossLabel:SetText("Boss: waiting (phase change)") end); task.wait(0.5)
                        end
                    end
                end
            end
...
task.spawn(function()
    while RUNNING do
        pcall(function()
            if inArena() then bossCountLabel:SetText("Boss: ACTIVE (in arena)")
            else
                local secs = 1800 - Workspace:GetServerTimeNow() % 1800
                bossCountLabel:SetText(("Next boss in ~%d:%02d"):format(math.floor(secs / 60), math.floor(secs % 60)))
            end
        end)
        task.wait(1)
    end
...
do
    local LvdBox = EventsTab:AddLeftGroupbox("Light vs Dark")
    LvdBox:AddToggle("se_lvd", { Text = "Auto Light vs Dark", Default = false })
    ...
    if LE then
        pcall(function() track(LE.Began.OnClientEvent:Connect(function(id, dur)
            if id == "LightVsDarkness" and type(dur) == "number" then
                ...
                pcall(function() Library:Notify({ Title = "COMBO_WICK", Description = "Light vs Dark STARTED -- " .. fmtT(dur) .. " left", Time = 6 }) end)
            end
        end)) end)
        ...
        while RUNNING do
            if on("se_lvd") and lvdActive() then
                if not lvdTeam() then
                    ...
                    pcall(function() lvdLabel:SetText("LvD: joining team") end); task.wait(0.5)
                else
                    ...
                    local left = lvdRemain(); local suffix = left > 0 and (" -- " .. fmtT(left) .. " left") or ""
                    pcall(function() lvdLabel:SetText("LvD: " .. rc .. " rings" .. suffix) end)
                    local tgt = lvdTarget()
                ...
                pcall(function() lvdLabel:SetText(on("se_lvd") and "LvD: waiting (staff-triggered, no timer)" or "LvD: idle") end)
                task.wait(1)
            end
        end
    end)
        ...
        local ok = pcall(function()
            ...
            for _, crys in ipairs(CollectionService:GetTagged("SakuraCrystal")) do
                ...
                if crys:GetAttribute("Owner") == lp.UserId then
                    ...
                    if okp and pos then
                        lockMove()
                        setStatus("Bloom: collecting crystal")
                        Nav.tweenTo3D(pos + Vector3.new(0, 3, 0), bloomGo, (Opt.se_speed and Opt.se_speed.Value) or 200)
    ...
    while RUNNING do
        if on("se_autofuse") then
            pcall(function()
                ...
                if not pick then setStatus("No 3-of-a-kind to fuse"); return end
...
ProtBox:AddToggle("se_antilag", { Text = "Reduce Lag", Default = false, Tooltip = "Kills particles/shadows/effects, low quality, and HIDES other players' pets (client-side) to stop the teleport lag spike", Callback = function(v) setAntiLag(v) end })
...
do
    ...
    local function setGuardNoclip(state)
        ...
        local areas = obj and obj:FindFirstChild("Areas")
    ...
    ProtBox:AddToggle("se_guardnoclip", { Text = "Guard Noclip", Default = true, Tooltip = "Makes guard animals non-collidable (client-side) so they can't physically block or shove you while stealing", Callback = function(v) setGuardNoclip(v) end })
    if Toggles.se_guardnoclip and Toggles.se_guardnoclip.Value then setGuardNoclip(true) end
end
ProtBox:AddDropdown("se_timelock", { Values = { "Off", "Day", "Night", "Sunrise", "Sunset" }, Default = "Off", Text = "Lock Time" })
...
MenuBox:AddLabel("Menu Toggle"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "Menu Toggle" })
...
local FG = SuggestTab:AddLeftGroupbox("Suggestions / Help", "message-square")
...
local HelpGroup = SuggestTab:AddRightGroupbox("Help / FAQ", "life-buoy")
HelpGroup:AddButton({
    Text = "Join Discord",
    Tooltip = DISCORD_INVITE,
    Func = function()
        pcall(function() setclipboard(DISCORD_INVITE) end)
        notify("Discord link copied. Paste it into your browser.")
    end,
})
