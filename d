...
Module.player.CharacterAdded:Connect(function(char)
	...
	if Module.currentTween then
		Module.currentTween:Cancel()
		Module.currentTween = nil
	end
...
function Module.isPlayerNearby(pos)
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= Module.player and p.Character then
			...
			if r and (pos - r.Position).Magnitude < Module.SAFETY_RADIUS then
				return true
			end
		end
	end
...
function Module.getAllItems()
	...
	for _, obj in ipairs(workspace:GetChildren()) do
		if Module.matchesTarget(obj.Name) then
			...
			if pos and not Module.isPlayerNearby(pos) then
...
function Module.farmAllItems()
	local currentTime = tick()
	
	if currentTime - Module.lastRefresh > 0.5 or #Module.allItems == 0 then
		Module.allItems = Module.getAllItems()
		Module.lastRefresh = currentTime
		Module.collectIndex = 1
	end
	
	if #Module.allItems == 0 then
		return nil
	end
	
	if Module.collectIndex > #Module.allItems then
		Module.collectIndex = 1
	end
	
	local targetPos = Module.allItems[Module.collectIndex]
	Module.collectIndex = Module.collectIndex + 1
...
function Module.tweenToPosition(targetPos)
	if not Module.rootPart or not Module.rootPart.Parent then return end
	
	if Module.currentTween then
		Module.currentTween:Cancel()
	end
	
	local distance = (Module.rootPart.Position - targetPos).Magnitude
	local duration = distance / Module.tweenSpeed
	...
	Module.currentTween = TweenService:Create(Module.rootPart, tweenInfo, goal)
	Module.currentTween:Play()
end
	...
	RunService.RenderStepped:Connect(function()
		...
		if Module.babyDinoActive then
			...
			if target then
				...
				if not targetDebounce or (lastTarget and (lastTarget - newPos).Magnitude > 5) then
					targetDebounce = true
					lastTarget = newPos
					Module.tweenToPosition(newPos)
					task.delay(0.5, function() targetDebounce = false end)
				end
			end
		...
		elseif Module.farmActive then
			local target = Module.farmAllItems()
			if target then
...
function Module.cleanup()
	...
	Module.camera.CameraType = Enum.CameraType.Custom
	Module.cameraCFrame = nil
	if Module.currentTween then Module.currentTween:Cancel() end
