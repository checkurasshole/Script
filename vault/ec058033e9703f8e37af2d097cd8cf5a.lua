-- Thai Translation
-- Script ID: ec058033e9703f8e37af2d097cd8cf5a
-- Updated: 2025-12-18T07:40:06.576Z
...
local Tabs = {
    Respawn = Window:AddTab("เกิดใหม่อัตโนมัติ", "refresh-cw"),
    AimAssist = Window:AddTab("ผู้ช่วยเล็ง", "star"),
    Visuals = Window:AddTab("ภาพ", "eye"),
    ["UI Settings"] = Window:AddTab("ตั้งค่า UI", "settings"),
}

local AutoPlayBox = Tabs.Respawn:AddLeftGroupbox("วนลูปเล่นอัตโนมัติ")
local AimbotV1Box = Tabs.AimAssist:AddLeftGroupbox("เวอร์ชัน 1 - PC แบบปุ่มกด")
local AimbotV2Box = Tabs.AimAssist:AddRightGroupbox("เวอร์ชัน 2 - อัตโนมัติ")
local TriggerBox = Tabs.AimAssist:AddLeftGroupbox("ยิงอัตโนมัติ")
local ESPBox = Tabs.Visuals:AddLeftGroupbox("ESP ผู้เล่น")
local TracerBox = Tabs.Visuals:AddRightGroupbox("เส้นติดตาม")

Module.Initialize(Library, Options, Toggles)

AutoPlayBox:AddToggle("AutoPlay", {
    Text = "วนลูปเล่นอัตโนมัติ",
    Default = false,
    Tooltip = "เกิดใหม่อัตโนมัติเมื่อตาย",
})
...
AimbotV1Box:AddToggle("AimbotV1", {
    Text = "เปิดใช้งาน",
    Default = false,
    Tooltip = "กดค้างคลิกขวาเพื่อล็อค",
})
...
AimbotV1Box:AddDropdown("AimbotV1Part", {
    Values = {"หัว", "ลำตัว", "จุดศูนย์กลาง"},
    Default = 1,
    Text = "ส่วนเป้าหมาย",
})

Options.AimbotV1Part:OnChanged(function()
    local mapping = {["หัว"] = "Head", ["ลำตัว"] = "Torso", ["จุดศูนย์กลาง"] = "HumanoidRootPart"}
...
AimbotV2Box:AddToggle("AimbotV2", {
    Text = "เปิดใช้งาน",
    Default = false,
    Tooltip = "ล็อคอัตโนมัติไม่ต้องกดปุ่ม",
})
                ...
                if player ~= LocalPlayer and Module.isAlive(player) then
                    ...
                    if hrp then
                        ...
                        if onScreen then
                            ...
                            local fromPos
                            if Options.TracerPosition.Value == "บน" then
                                fromPos = Vector2.new(Camera.ViewportSize.X / 2, 0)
                            elseif Options.TracerPosition.Value == "กลาง" then
